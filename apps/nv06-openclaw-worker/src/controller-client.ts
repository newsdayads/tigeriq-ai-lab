...
import { asRecord,NV06_CAPABILITIES,NV06_PERMISSIONS,sha256,stringValue,type Identity,type WorkerLease } from './types.js';
...
export class ControllerClient {
  ...
  async renew(lease:WorkerLease):Promise<void>{await this.signed(`/api/v1/jobs/${encodeURIComponent(lease.job.jobId)}/lease/renew`,{leaseId:lease.leaseId,leaseToken:lease.leaseToken,leaseTtlMs:300_000});}
  ...
  private async signed(resource:string,body:Record<string,unknown>):Promise<Record<string,unknown>>{
    const raw=JSON.stringify(body),rawBytes=Buffer.from(raw,'utf8'),timestamp=String(Date.now()),nonce=randomBytes(16).toString('hex'),bodyHash=sha256(rawBytes);
    const canonical=`POST\n${resource}\n${this.identity.employeeId}\n${this.identity.nodeId}\n${this.identity.deviceId}\n${timestamp}\n${nonce}\n${bodyHash}`;
    const challenge=sha256(canonical),signature=signPayload('sha256',Buffer.from(canonical,'utf8'),createPrivateKey(this.identity.privateKeyPem)).toString('base64url');
  ...
  private async requestRaw(resource:string,raw:string,headers:Record<string,string>):Promise<Record<string,unknown>>{
    const response=await fetch(new URL(resource,this.baseUrl),{method:'POST',headers:{'Content-Type':'application/json',...headers},body:raw});
