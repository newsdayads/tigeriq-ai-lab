...
export class NV06Worker {
  ...
  private async executeLease(lease:WorkerLease):Promise<void>{
    if(!this.client)return;const runtimeDeviceId=this.client.identity.deviceId,startedAt=new Date().toISOString(),renewErrors:string[]=[];let renewFailed=false;
    ...
    try{
      ...
      const completedAt=new Date().toISOString(),output={route:'browser.chatgpt',worker:NV06_EMPLOYEE_ID,device:runtimeDeviceId,sessionKey:run.sessionKey,browserUsed:run.browserUsed,durationMs:run.durationMs,text:run.text};
      const stored=await this.persistEvidence(lease.job,runtimeDeviceId,startedAt,completedAt,'completed',output,renewErrors);
      ...
      const stored=await this.persistEvidence(lease.job,runtimeDeviceId,startedAt,completedAt,'failed',{code,message},renewErrors);
  ...
  private async persistEvidence(job:WorkerJob,runtimeDeviceId:string,startedAt:string,completedAt:string,status:'completed'|'failed',output:Record<string,unknown>,renewErrors:string[]):Promise<{ref:string;digest:string}>{
    ...
    const absolute=path.join(dir,`${Date.now()}-nv06-${status}.json`),document={work_order_id:job.jobId,worker:NV06_EMPLOYEE_ID,device:runtimeDeviceId,started_at:startedAt,completed_at:completedAt,selected_route:'browser.chatgpt',output_result:output,lease_renew_errors:renewErrors,final_status:status};
