...
async function runJob(j){
  ...
  const rememberCooldowns=ledger=>{for(const row of ledger||[])if(row?.class==='rate_limit'&&row?.resourceId&&!cooldownExcludes.includes(row.resourceId))cooldownExcludes.push(row.resourceId)};
  ...
  if(shouldResumeExistingPr(j)){
    ...
    generated=await generateChanges(worker,j,context,[],cooldownExcludes);rememberCooldowns(generated.failureLedger);worker=generated.resource;gen=generated.payload;
  ...
  for(let reviewCycle=0;reviewCycle<3;reviewCycle++){
    gates=await runGateWithRepair({
      ...
      repairFn:async({evidence})=>{
        ...
        rememberCooldowns(generated.failureLedger);worker=generated.resource;gen=generated.payload;
    ...
    const reviewed=await reviewPr(reviewer,j,diff,worker.id,cooldownExcludes);rememberCooldowns(reviewed.failureLedger);reviewer=reviewed.resource;review=reviewed.review;
    ...
    context=await contextFor(j.paths,branch);
    generated=await generateRepairEdits(worker,j,context,review.issues,[reviewer.id,...cooldownExcludes]);rememberCooldowns(generated.failureLedger);worker=generated.resource;gen=generated.payload;
    if(reviewer.id===worker.id){reviewer=pickResource([worker.id,...cooldownExcludes]);if(!reviewer)throw new Error('NO_INDEPENDENT_REVIEWER_AVAILABLE')}
    await pool.query("update tigeriq_coding_jobs set employee_id=$2,reviewer_employee_id=$3,status='waiting_ci' where id=$1",[j.id,worker.id,reviewer.id]);
    await writeRepairEdits(branch,gen.edits);
  }
