...
import {parseEnvelopeOrLegacy, wrapFileChange} from './envelope.mjs';
...
export function validateJobScope(jobPaths,changes){
  ...
  const normalizedChanges=(changes||[]).map(c=>{
    try { return parseEnvelopeOrLegacy(c); } catch { return c; }
  });
  const offending=normalizedChanges.map(c=>c?.path).filter(Boolean).filter(p=>!allowed.has(p));
...
jobs set employee_id=$2,reviewer_employee_id=$3,status='waiting_ci' where id=$1",[j.id,worker.id,reviewer.id]);
