...
import {Pool} from 'pg';
import {branchName,checkGateState,extractCanonicalAllowedPaths,isRetryableAiError,parseJsonObject,safeRepoPath,validateChanges,validateCompactContract} from './policy.mjs';
...
async function generateAndWriteRepair(worker,j,branch,issues=[],exclude=[]){
  ...
  const code='COMPACT_REPAIR_EXHAUSTED';
  const e=new Error(code);
  e.code=code;
  e.detail={last,edits:(last?.payload?.edits||[]).length,attempts:(last?.payload?.summary||'').slice(0,50)};
  throw e;
}
