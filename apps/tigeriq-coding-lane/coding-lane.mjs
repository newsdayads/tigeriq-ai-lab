...
const DATABASE_URL=process.env.DATABASE_URL?.trim(); if(!DATABASE_URL) throw new Error('DATABASE_URL_MISSING');
const GH_TOKEN=(process.env.TIGERIQ_GITHUB_TOKEN||process.env.GITHUB_TOKEN||'').trim(); if(!GH_TOKEN) throw new Error('GITHUB_TOKEN_MISSING');
...
if(!resources.length) throw new Error('NO_FREE_API_CODING_RESOURCE');
let rr=0;
...
function isRetryableError(err) {
  ...
  if (
    ...
    msg.includes('JSON_OBJECT_INVALID') ||
...
async function invokeWithRetry(initialResource, prompt, opName = 'MODEL_INVOCATION') {
  let currentResource = initialResource;
  const triedIds = [];
  if (currentResource) triedIds.push(currentResource.id);

  for (let attempt = 1; attempt <= 3; attempt++) {
    if (!currentResource) {
      currentResource = pickResource(triedIds);
      if (!currentResource) {
        throw new Error(`${opName}_NO_RESOURCE_AVAILABLE`);
      }
      if (!triedIds.includes(currentResource.id)) {
        triedIds.push(currentResource.id);
      }
    }

    try {
      const raw = await invoke(currentResource, prompt);
      let parsed;
      try {
        parsed = parseJsonObject(raw);
      } catch (parseErr) {
        const errObj = new Error('JSON_OBJECT_INVALID');
        errObj.detail = String(parseErr.message || parseErr);
        throw errObj;
      }
      return { parsed, resource: currentResource };
    } catch (err) {
      if (attempt < 3 && isRetryableError(err)) {
        currentResource = pickResource(triedIds);
        if (currentResource && !triedIds.includes(currentResource.id)) {
          triedIds.push(currentResource.id);
        }
        continue;
      }
  ...
  throw new Error(`${opName}_EXHAUSTED`);
}
...
async function managerTick(){
  ...
  const o=q.rows[0];
  if(!o) return;
  const manager=pickResource();
  if(!manager){
    await pool.query("update tigeriq_coding_objectives set status='blocked',summary='NO_FREE_API_CODING_RESOURCE' where id=$1",[o.id]);
    return;
  }
  ...
  let d, usedManager;
  try {
    const res = await invokeWithRetry(manager, prompt, 'MANAGER_TICK');
    d = res.parsed;
    usedManager = res.resource;
  } catch (e) {
    await pool.query("update tigeriq_coding_objectives set status='blocked',summary=$2,manager_employee_id=$3,updated_at=now() where id=$1",[o.id,String('BLOCKED_TECHNICAL: ' + (e.message || e)).slice(0,1000),manager.id]);
    return;
  }

  try {
    if(d.status!=='continue'||!d.job){
      await pool.query("update tigeriq_coding_objectives set status='blocked',summary=$2,manager_employee_id=$3,updated_at=now() where id=$1",[o.id,String(d.summary||'manager blocked').slice(0,1000),usedManager.id]);
      return;
    }
    const paths=[...new Set((d.job.paths||[]).map(String))].filter(safeRepoPath).slice(0,8);
    if(!paths.length) throw new Error('MANAGER_PATHS_EMPTY');
    ...
    await pool.query("update tigeriq_coding_objectives set manager_employee_id=$2,summary=$3,updated_at=now() where id=$1",[o.id,usedManager.id,String(d.summary||'coding job created').slice(0,1000)]);
  } catch(e){
    await pool.query("update tigeriq_coding_objectives set status='blocked',summary=$2,manager_employee_id=$3,updated_at=now() where id=$1",[o.id,String('BLOCKED_TECHNICAL: ' + e.message).slice(0,1000),usedManager.id]);
  }
}
...
async function generateChanges(worker,j,context,reviewIssues=[]){
  ...
  const res = await invokeWithRetry(worker, prompt, 'GENERATE_CHANGES');
  validateChanges(res.parsed.changes,j.paths);
  return res.parsed;
}

async function reviewPr(reviewer,j,diff){
  const prompt=`You are ${reviewer.id}, independent TigerIQ code reviewer. Review against the task and safety boundaries. TASK: ${j.instruction}\nDIFF:\n${diff.slice(0,180000)}\nReturn ONLY JSON {"decision":"approve|changes_requested","summary":"short","issues":["specific issue"]}. Reject unsafe, untested, out-of-scope, credential/security/production changes.`;
  const res = await invokeWithRetry(reviewer, prompt, 'REVIEW_PR');
  const d = res.parsed;
  if(!['approve','changes_requested'].includes(d.decision))throw new Error('REVIEW_DECISION_INVALID');
  d.issues=Array.isArray(d.issues)?d.issues.slice(0,8):[];
  return d;
}

async function runJob(j){
  const worker=pickResource();
  if(!worker)throw new Error('NO_IMPLEMENTER_AVAILABLE');
  const reviewer=pickResource([worker.id]);
  ...
  const branch=branchName(worker.id,j.id);
  ...
  let gen;
  try {
    gen=await generateChanges(worker,j,context);
  } catch(e) {
    throw Object.assign(new Error(`BLOCKED_TECHNICAL: ${e.message || e}`), { detail: e.detail || null });
  }
...
Direct writes to main are forbidden. Merge is attempted only after CI gates and reviewer approval.`);
  ...
  for(let cycle=0;cycle<3;cycle++){
    ...
    try {
      review=await reviewPr(reviewer,j,diff);
    } catch(e) {
      throw Object.assign(new Error(`BLOCKED_TECHNICAL: ${e.message || e}`), { detail: e.detail || null });
    }
  ...
  if(AUTO_MERGE){
    try{merge=await mergePr(pr.number,finalSha)}catch(e){merge={merged:false,message:String(e.message||e)}}
  }
...
async function failJob(j,e){
  ...
  const finalMsg = msg.includes('EXHAUSTED') || msg.includes('JSON') || msg.includes('EMPTY_RESPONSE') || msg.includes('HTTP_') || msg.includes('BLOCKED_TECHNICAL') ? (msg.startsWith('BLOCKED_TECHNICAL') ? msg : `BLOCKED_TECHNICAL: ${msg}`) : msg;
  await pool.query("update tigeriq_coding_jobs set status='failed',failure=$2,completed_at=now() where id=$1",[j.id,JSON.stringify({message:finalMsg,detail:e?.detail||null})]);
  await pool.query("update tigeriq_coding_objectives set status='blocked',summary=$2,updated_at=now() where id=$1",[j.objective_id,finalMsg.slice(0,1000)]);
}
...
export { invokeWithRetry, isRetryableError, managerTick, runJob, failJob };
