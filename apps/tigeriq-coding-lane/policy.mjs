...
export function isRetryableAiError(error){
  ...
  return error?.name==='AbortError'||/MANAGER_(?:SOFT_BLOCK|SCOPE_MISMATCH|PATHS_INVALID)|CODING_CHANGES_COUNT_INVALID|CODING_COMPACT_(?:EDIT|EDITS)[A-Z0-9_]*|COMPACT_EDIT_[A-Z0-9_]+|JSON_OBJECT_(?:INVALID|MISSING)|unterminated|truncat|schema|EMPTY_RESPONSE|fetch failed|aborted|ECONNRESET|ETIMEDOUT|socket|HTTP_(?:408|409|413|429|500|502|503|504)\b/i.test(msg);
}
