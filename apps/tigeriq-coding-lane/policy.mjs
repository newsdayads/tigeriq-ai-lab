...
export function isRetryableAiError(error){
  ...
  return error?.name==='AbortError'||/CODING_CHANGES_COUNT_INVALID|JSON_OBJECT_(?:INVALID|MISSING)|MUTATION_ENVELOPE_|unterminated|truncat|schema|EMPTY_RESPONSE|fetch failed|aborted|ECONNRESET|ETIMEDOUT|socket|HTTP_(?:408|409|413|429|500|502|503|504)\b/i.test(msg);
}
