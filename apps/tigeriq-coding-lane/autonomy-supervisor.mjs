...
export function classifyIssueAutonomy(issue){
  if(!issue||typeof issue!=='object')return{eligible:false,reason:'ISSUE_UNAVAILABLE'};
  if(issue.pull_request)return{eligible:false,reason:'ISSUE_IS_PR'};
  if(String(issue.state||'').toLowerCase()!=='open')return{eligible:false,reason:'ISSUE_NOT_OPEN'};
...
async function queueRetry(pool,job,reason,maxJobs){
  ...
  if(!shouldRetry(total,maxJobs)){
    await pool.query("update tigeriq_coding_objectives set status='blocked',summary=$2,updated_at=now() where id=$1",[job.objective_id,`BLOCKED:AUTO_REPAIR_EXHAUSTED:${reason}`]);
    await emit(pool,'BLOCKED',{objectiveId:job.objective_id,jobId:job.id,reason,totalJobs:total,status:'BLOCKED'});
    return false;
  }
...
async function handleFailed(pool,maxJobs){
  ...
  for(const job of q.rows){
    ...
    if(!gate.eligible){
      await pool.query("update tigeriq_coding_objectives set status='blocked',summary=$2,updated_at=now() where id=$1",[job.objective_id,`BLOCKED:AUTO_REPAIR_INELIGIBLE:${gate.reason}`]);
      await emit(pool,'REPAIR_SKIPPED',{objectiveId:job.objective_id,jobId:job.id,reason:'AUTO_REPAIR_INELIGIBLE',eligibilityReason:gate.reason,issueNumber:gate.issueNumber});
      continue;
    }
...
async function handleStale(pool,staleMs,maxJobs,onStall){
  ...
  for(const job of q.rows){
    ...
    const gate=await objectiveIsEligible(pool,job.objective_id);
    if(!gate.eligible){
      await pool.query("update tigeriq_coding_jobs set status='failed',failure=$2,completed_at=now() where id=$1",[job.id,JSON.stringify({message:'AUTO_REPAIR_REVOKED',code:gate.reason,supervisorHandled:true,previousStatus:job.status})]);
      await pool.query("update tigeriq_coding_objectives set status='blocked',summary=$2,updated_at=now() where id=$1",[job.objective_id,`BLOCKED:AUTO_REPAIR_INELIGIBLE:${gate.reason}`]);
      await emit(pool,'REPAIR_SKIPPED',{objectiveId:job.objective_id,jobId:job.id,reason:'AUTO_REPAIR_INELIGIBLE',eligibilityReason:gate.reason,issueNumber:gate.issueNumber,previousStatus:job.status});
      continue;
    }
...
,'mi'));
  ...
  if(String(field('TIGERIQ_EXECUTABLE')||'').toLowerCase()!=='true')return{eligible:false,reason:'TIGERIQ_EXECUTABLE_NOT_TRUE'};
  if(String(field('OWNER_POLICY')||'').toUpperCase()!=='AUTO')return{eligible:false,reason:'OWNER_POLICY_NOT_AUTO'};
  if(String(field('NV02_SELF_MODIFICATION_GUARD')||'').toLowerCase()==='true')return{eligible:false,reason:'NV02_SELF_MODIFICATION_GUARD'};
  const state=String(field('STATE')||'').toUpperCase();
  if(state.includes('OWNER_HOLD')||state.includes('MANUAL_HOLD'))return{eligible:false,reason:'OWNER_HOLD'};
  return{eligible:true,reason:'AUTO_ALLOWED'};
}
export async function githubIssueEligibility(issueNumber,fetchImpl=fetch,tokenOverride=null){
  const token=String(tokenOverride??process.env.TIGERIQ_GITHUB_TOKEN??process.env.GITHUB_TOKEN??'').trim();
  if(!token||!issueNumber)return{eligible:false,reason:!issueNumber?'ISSUE_NUMBER_MISSING':'GITHUB_TOKEN_MISSING',issueNumber};
  try{
    ...
    if(!res.ok)return{eligible:false,reason:`GITHUB_HTTP_${res.status}`,issueNumber};
    const issue=await res.json();
    return{...classifyIssueAutonomy(issue),issueNumber};
  }catch(error){
    return{eligible:false,reason:'GITHUB_READ_FAILED',issueNumber,error:String(error?.message||error)};
  }
}
async function objectiveIsEligible(pool,objectiveId,fetchImpl=fetch){
  ...
  return githubIssueEligibility(issueNumber,fetchImpl);
}
async function countObjectiveJobs(pool,objectiveId){
  const q=await pool.query('select count(*)::int n from tigeriq_coding_jobs where objective_id=$1',[objectiveId]);
  return Number(q.rows[0]?.n||0);
