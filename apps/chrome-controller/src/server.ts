...
const selfRunClaimPath=resolve(config.logDir,'appchrome-self-run-claims.json');
...
const selfRunClaims=new DurableSelfRunClaimStore(selfRunClaimPath);
...
function clearPending(state:DurableAutopilotState):DurableAutopilotState{
  const {pendingJobId:_pendingJobId,pendingWorkerId:_pendingWorkerId,pendingReservedAt:_pendingReservedAt,...rest}=state;
  return rest;
}
...
function issueNumberFromRef(ref:string|null|undefined){
  ...
  for(const url of [selfRunCoreStatusUrl,selfRunCodingStatusUrl]){
    try{
      ...
      for(const objective of objectives){
        if(!['queued','active','running','waiting_ci','reviewing','waiting_resource'].includes(String(objective?.status||'').toLowerCase()))continue;
        collectResourceScopes(objective,scopes);
      }
    }catch(error){
      log('APP_CHROME_SELF_RUN_SCOPE_SOURCE_UNAVAILABLE',{url,error:String(error)});
    }
  }
  return scopes;
}
function completeSelfRunJob(workerId:WorkerId,jobId:string,evidenceRef:string,result:string){
  let job=uiJobLedger.get(workerId,jobId);
  if(!job||isTerminalUiJobStage(job.stage))return;
  if(job.stage==='SUBMITTED'||job.stage==='WORKING')job=uiJobLedger.transition(workerId,jobId,'WAITING_EVIDENCE',{evidenceRef,nextAction:'Verify GitHub terminal state'});
...
async function reconcileSelfRunWorker(workerId:WorkerId){
  ...
  if(!active||active.source!=='APP_CHROME_SELF_RUN'||!['SUBMITTED','WORKING','WAITING_EVIDENCE','VERIFY'].includes(active.stage))return false;
  ...
  const historical=appChromeClaimForJob(comments,workerId,issueNumber,active.jobId);
  const claim=localClaim??historical?.claim;
  const claimId=claim?.claimId??'';
  if(issue.state!=='closed'&&!claimId)return false;

  const terminal=authoritativeUiTerminalFromGithub(issue,comments,claimId);
  ...
  if(terminal==='EXTERNAL_WAIT'){
    ...
    log('APP_CHROME_SELF_RUN_EXTERNAL_WAIT',{workerId,jobId:active.jobId,issueNumber,claimId:claimId||null});
    persistEvidence();
    return true;
  }

  if(claim){
    if(historical?.released===true){
      selfRunClaims.release(claim.claimId);
      ...
      if(!selfRunGithubToken){
        log('APP_CHROME_SELF_RUN_CLAIM_RELEASE_DEFERRED',{workerId,issueNumber,claimId:claim.claimId,state:terminal,error:'APP_CHROME_GITHUB_TOKEN_REQUIRED'});
        return false;
      }
  ...
  if(terminal==='DONE'&&issue.state!=='closed'){
    if(!selfRunGithubToken)return false;
  ...
  uiJobLedger.reconcileAuthoritativeTerminal(workerId,active.jobId,terminal,{
    ...
    blocker:terminal==='BLOCKED'
      ?(issue.state==='closed'?`SOURCE_ISSUE_CLOSED_${String(issue.state_reason||'UNKNOWN').toUpperCase()}`:'BLOCKED')
      :null,
    result:terminal==='DONE'
      ?`GitHub authoritative terminal state DONE for issue #${issueNumber}; self-run assignment cleared.`
      :(issue.state==='closed'
        ?`GitHub issue #${issueNumber} closed without completed state.`
        :'GitHub authoritative terminal evidence is BLOCKED.'),
  });
  log('APP_CHROME_SELF_RUN_TERMINAL_RECONCILED',{workerId,jobId:active.jobId,issueNumber,terminal,claimId:claimId||null,stateReason:issue.state_reason??null});
  persistEvidence();
  return true;
}
    ...
    for(const workerId of WORKER_IDS){
      ...
      try{
        ...
        const comments=issue.state==='closed'
          ?[]
          :await fetchIssueComments({issueNumber,owner:selfRunGithubOwner,repo:selfRunGithubRepo,token:selfRunGithubToken});
        const terminal=authoritativeUiTerminalFromGithub(issue,comments);
        ...
        const evidenceRef=issue.html_url;
        if(terminal==='EXTERNAL_WAIT'){
          uiJobLedger.reconcileAuthoritativeTerminal(workerId,jobId,'EXTERNAL_WAIT',{
            evidenceRef,
            nextAction:'External wait; preserve current assignment without continue spam',
            blocker:null,
            result:'GitHub authoritative terminal marker is EXTERNAL_WAIT',
          });
          log('UI_JOB_GITHUB_EXTERNAL_WAIT_RECONCILED',{workerId,jobId,issueNumber,evidenceRef});
        }else{
          uiJobLedger.reconcileAuthoritativeTerminal(workerId,jobId,terminal,{
            evidenceRef,
            blocker:terminal==='BLOCKED'
              ?(issue.state==='closed'?`SOURCE_ISSUE_CLOSED_${String(issue.state_reason||'UNKNOWN').toUpperCase()}`:'BLOCKED')
              :null,
            result:terminal==='DONE'
              ?`GitHub authoritative terminal state DONE for issue #${issueNumber}; terminal state reconciled.`
              :(issue.state==='closed'
                ?`GitHub issue #${issueNumber} closed without completed state.`
                :'GitHub authoritative terminal evidence is BLOCKED.'),
          });
          log('UI_JOB_GITHUB_TERMINAL_RECONCILED',{workerId,jobId,issueNumber,terminal,evidenceRef,stateReason:issue.state_reason??null});
        }
        persistEvidence();
      }catch(error){
...
async function selfRunTick(){
  ...
  selfRunTicking=true;
  selfRunState.lastTickAt=new Date().toISOString();
  try{
    ...
    for(const workerId of WORKER_IDS){
      ...
      const issueNumber=issueNumberFromRef(active.issueRef);
      if(issueNumber){
        excludedIssues.add(issueNumber);
        try{
          const issue=issues.find((x)=>x.number===issueNumber)??await fetchGithubIssue({issueNumber,owner:selfRunGithubOwner,repo:selfRunGithubRepo,token:selfRunGithubToken});
          blockedScopes.add(resourceScopeOf(issue));
        }catch{}
      }
    }
    for(const workerId of WORKER_IDS){
      ...
      const state=states.get(workerId)!;
      if(!state.enabled||state.blocked||utilityPausedWorkers.has(workerId)||state.manualCloseSuppressed)continue;
      if(!recentHeartbeat(workerId)||state.lastHeartbeat?.uiBusy!==false)continue;
      const security=heartbeatStopReason(state.lastHeartbeat);
      if(security)continue;
      const candidates=eligibleIssuesForWorker(workerId,issues,blockedScopes,excludedIssues).slice(0,12);
      for(const issue of candidates){
        const local=selfRunClaims.acquire({issueNumber:issue.number,scope:resourceScopeOf(issue),workerId,ttlMs:6*60*60*1000});
        if(local.kind==='BUSY')continue;
        const claim=await claimGithubIssue({
        ...
        try{
          ...
          const jobId=`APP-GH-${issue.number}-${workerId}-${claim.claimId.slice(0,8)}`;
          await dispatch(workerId,prompt,false,'MANUAL',{
            jobId,
          ...
          blockedScopes.add(resourceScopeOf(issue));
          excludedIssues.add(issue.number);
          selfRunState.lastClaimAt=new Date().toISOString();
          selfRunState.lastClaimIssue=issue.number;
    ...
    selfRunState.lastError=null;
  }catch(error){
    selfRunState.lastError=String(error);
    log('APP_CHROME_SELF_RUN_ERROR',{error:String(error)});
  }finally{
    selfRunTicking=false;
  }
}
function scheduleSelfRunTick(delayMs=5000){
  if(selfRunTimer)clearTimeout(selfRunTimer);
  selfRunTimer=setTimeout(async()=>{
    await selfRunTick();
    const next=20_000+Math.floor(Math.random()*20_001);
    scheduleSelfRunTick(next);
  },delayMs);
  selfRunTimer.unref();
}
...
function reconcileCompletedUiJobFromSnapshot(){
  ...
  if(!previous||previous.status!=='DONE')return false;
  const workerId=previous.workerId;
  ...
  if(autopilotState.lastCompletedJobId!==previous.jobId){
    ...
    persistAutopilotState();
    log('COMPLETION_WATCHER_DONE_EVIDENCE',{workerId,jobId:previous.jobId,evidenceRef:ext.ref,evidenceRevision:ext.completionRevision,source:ext.source});
  }
  let active=uiJobLedger.active(workerId);
  if(active?.jobId!==previous.jobId)return true;
  if(active.stage==='SUBMITTED'||active.stage==='WORKING'){
    uiJobLedger.transition(workerId,previous.jobId,'WAITING_EVIDENCE',{evidenceRef:ext.ref,nextAction:'Verify authoritative completion'});
    active=uiJobLedger.active(workerId);
  }
  if(active?.jobId===previous.jobId&&active.stage==='WAITING_EVIDENCE'){
    uiJobLedger.transition(workerId,previous.jobId,'VERIFY',{evidenceRef:ext.ref,nextAction:'Verify authoritative completion'});
    uiJobLedger.transition(workerId,previous.jobId,'DONE',{evidenceRef:ext.ref,result:`Authoritative external completion verified for ${previous.jobId}`});
    log('UI_JOB_EXTERNAL_COMPLETION_RECONCILED',{workerId,jobId:previous.jobId,evidenceRef:ext.ref});
  }
  return true;
}
...
async function autopilotTick(){
  ...
  try{
    ...
    if(autopilotState.pendingJobId){
      ...
      const pendingWorkerId=autopilotState.pendingWorkerId??latestSnapshot?.nextJob?.workerId??'NV02';
      ...
      if(canRetireWithdrawn){
        const primary=states.get(pendingWorkerId)!;
        if(!recentHeartbeat(pendingWorkerId)||primary.lastHeartbeat?.uiBusy!==false){
          stopAutopilot(`PENDING_SOURCE_WITHDRAWN_UI_NOT_IDLE:${pendingJobId}`);persistEvidence();return;
        }
        const localJob=uiJobLedger.get(pendingWorkerId,pendingJobId);
        if(localJob&&!isTerminalUiJobStage(localJob.stage))uiJobLedger.transition(pendingWorkerId,pendingJobId,'BLOCKED',{nextAction:null,blocker:'SOURCE_JOB_NO_LONGER_EXECUTABLE',result:'Source withdrew/cancelled work before a committed dispatch'});
        const retired=dispatchLease.retireNoLongerExecutable(pendingJobId,Date.now());
        autopilotState={...clearPending(autopilotState),phase:'IDLE',uncertainJobId:undefined,uncertainWorkerId:undefined,dispatchFailureClass:undefined,retryAt:undefined,updatedAt:new Date().toISOString()};
        persistAutopilotState();
        ...
        if(reconciliation.kind==='COMMITTED'){
          autopilotState={...clearPending(autopilotState),phase:'BUSY',lastDispatchedJobId:pendingJobId,lastDispatchedWorkerId:pendingWorkerId,lastDispatchedAt:reconciliation.lease.dispatchedAt,uncertainJobId:undefined,uncertainWorkerId:undefined,updatedAt:new Date().toISOString()};
    ...
    if(autopilotState.uncertainJobId){
      ...
      const uncertain=uiJobLedger.get(uncertainWorkerId,uncertainJobId);
      const leaseState=dispatchLease.read();
      ...
      if(knownNotDelivered||orphanBeforeLedgerCommit){
        ...
        autopilotState={...clearPending(autopilotState),phase:'IDLE',uncertainJobId:undefined,uncertainWorkerId:undefined,dispatchFailureClass:'SAFE_RETRY',retryAt:new Date().toISOString(),updatedAt:new Date().toISOString()};
        persistAutopilotState();
    ...
    const primary=states.get(workerId)!;
    if(utilityPausedWorkers.has(workerId)){setAutopilotPhase('IDLE');persistEvidence();return;}
    if(!primary.enabled||primary.blocked||!startupReady){stopAutopilot(primary.blocked?`${workerId}_BLOCKED`:`${workerId}_NOT_READY`);persistEvidence();return;}
    if(!recentHeartbeat(workerId)){setAutopilotPhase('RECOVERING');persistEvidence();return;}
    ...
    try{
      ...
      await dispatch(workerId,decision.text,false,'AUTO_CONTINUE',{jobId:decision.jobId,issueRef:decision.issueRef,title:`Core UI ${decision.jobId}`,source:'CORE_UI'});
      ...
      if(failureClass==='SAFE_RETRY'){
        try{
          ...
          autopilotState={...clearPending(autopilotState),phase:'STOPPED',uncertainJobId:decision.jobId,uncertainWorkerId:workerId,dispatchFailureClass:'UNCERTAIN',retryAt:undefined,updatedAt:new Date().toISOString()};
          persistAutopilotState();
          log('AUTO_CONTINUE_SAFE_RETRY_STATE_FAILED',{jobId:decision.jobId,error:String(retryError),originalError:message});
        }
      }else{
        autopilotState={...clearPending(autopilotState),phase:'STOPPED',uncertainJobId:decision.jobId,uncertainWorkerId:workerId,updatedAt:new Date().toISOString()};
        persistAutopilotState();
...
async function recoveryTick(){
  ...
  try{
    await reconcileGithubTerminalUiJobs();
    for(const id of WORKER_IDS){
      const state=states.get(id)!;
...
async function handleApi(req:IncomingMessage,res:ServerResponse,url:URL):Promise<boolean>{
  ...
  if(url.pathname==='/api/workers/NV02/restart-schedule'&&req.method==='POST'){
    try{
      ...
      const boundedRecovery=stalledRecovery;
      ...
      if(reason==='WORKING_NO_PROGRESS_3_CHECKS')throw new Error('NV02_WORKING_RESTART_FORBIDDEN');
  ...
  if(utilityMatch){
    ...
    try{
      ...
      if(action==='safe-recover'){
        ...
        const presence=await brokerWorkerPresence(workerId);
        if(presence==='AMBIGUOUS')throw new Error(`RECOVERY_AMBIGUOUS_WINDOW:${workerId}`);
        if(presence==='RUNNING'){
          state.status='RECOVERY_ATTACH_WAIT';
          state.lastError=undefined;
          log('SAFE_RECOVER_ATTACH_EXISTING_STALE_HEARTBEAT',{workerId,lastWindowId:state.lastWindowId??null});
          persistEvidence();
          json(res,202,{ok:true,mode:'ATTACH_EXISTING_STALE_HEARTBEAT'});
          return true;
        }
        state.windowState='CLOSED';
        state.lastError=undefined;
        await launchChrome(workerId);
        log('SAFE_RECOVER_BROKER_LAUNCH_REQUESTED',{workerId});
        persistEvidence();
