...
async function handleApi(req:IncomingMessage,res:ServerResponse,url:URL):Promise<boolean>{
  ...
  if(url.pathname==='/api/result'&&req.method==='POST'){
    ...
    if(String(data.status??'').startsWith('BLOCKED')){
      const state=states.get(workerId);
      if(String(data.status??'').includes('MODEL_PROFILE_BLOCKED')){
      const state=states.get(workerId);
      if(state){state.blocked=true;if(state.enabled)state.status='MODEL_PROFILE_BLOCKED';state.lastError='MODEL_PROFILE_BLOCKED';}
      if(workerId==='NV02')stopAutopilot('MODEL_PROFILE_BLOCKED');
    } else if(state){state.blocked=true;if(state.enabled)state.status='BLOCKED';state.lastError=String(data.status);}
    if(workerId==='NV02' && String(data.status??'').includes('MODEL_PROFILE_BLOCKED'))stopAutopilot('MODEL_PROFILE_BLOCKED');
