...
export function configFromEnv():NativeWorkerConfig{
  ...
  return {workspace,identityFile:path.join(stateRoot,'identity.json'),controllerUrl:process.env.TIGERIQ_CONTROLLER_URL?.trim()||'http://100.97.23.87:8790',ingressToken,ollamaEndpoint:process.env.TIGERIQ_OLLAMA_URL?.trim()||'http://127.0.0.1:11434',ollamaModel:process.env.TIGERIQ_OLLAMA_MODEL?.trim()||'qwen3:4b',pollMs:Math.max(250,Number(process.env.TIGERIQ_WORKER_POLL_MS??1000)),heartbeatMs:Math.max(5000,Number(process.env.TIGERIQ_HEARTBEAT_MS??15000)),maxConcurrentJobs:Math.min(8,Math.max(1,Number(process.env.TIGERIQ_WORKER_MAX_JOBS??4))),minFreeRamBytes:Math.max(4,Number(process.env.TIGERIQ_MIN_FREE_RAM_GB??8))*1024**3};
}
