...
/** PC01 control-plane client only. It never sends prompts to PC01 for AI execution. */
public final class TigerIqApiClient {
    public static final String SESSION_PATH = "/v1/android/sessions", PULL_PATH = "/v1/android/jobs/pull", SUBMIT_PATH = "/v1/android/jobs/submit";
    ...
    private final DeviceKeyStore deviceKey;
    ...
    public TigerIqApiClient(EmployeeDeviceStore.Profile profile) {
        ...
        this.deviceKey = new DeviceKeyStore(profile.employeeId, profile.deviceId);
    }
    ...
    public Session mintSession(String bootstrapToken) throws Exception {
        JSONObject body = new JSONObject()
                ...
                .put("requestedScopes", new JSONArray().put("android:jobs:pull").put("android:jobs:submit"))
                .put("client", new JSONObject().put("name", "tigeriq-android-ai-employee").put("version", "1.0.0"));
        JSONObject response = request("POST", SESSION_PATH, body, Headers.bootstrap(profile.credentialId, bootstrapToken), 30_000);
        JSONObject session = response.optJSONObject("session");
        if (session == null) throw new ApiException(502, "INVALID_RESPONSE", "missing session response", false, null);
    ...
    public PullResult pullJob(String sessionToken) throws Exception {
        JSONObject response = request("POST", PULL_PATH, new JSONObject().put("employeeId", profile.employeeId).put("deviceId", profile.deviceId), Headers.session(sessionToken), 30_000);
        JSONObject payload = unwrapData(response);
        String kind = payload.optString("kind", "");
        if ("empty".equals(kind)) return PullResult.empty();
        JSONObject lease = payload.optJSONObject("lease");
        if (!"job".equals(kind) || lease == null) throw new ApiException(502, "INVALID_RESPONSE", "invalid pull response", false, null);
        ...
        if (profile.bindingId != null && !profile.bindingId.isBlank() && !profile.bindingId.equals(bindingId)) throw new ApiException(409, "STALE_BINDING", "job lease binding mismatch", false, null);
        JSONObject job = lease.optJSONObject("job");
        if (job == null) throw new ApiException(502, "INVALID_RESPONSE", "lease missing job", false, null);
        String jobId = required(lease, "jobId");
        if (!jobId.equals(required(job, "jobId"))) throw new ApiException(409, "IDENTITY_MISMATCH", "lease job mismatch", false, null);
        return PullResult.job(new DurableCheckpointStore.JobLease(jobId, required(job, "idempotencyKey"), bindingId, required(lease, "leaseId"), required(lease, "leaseToken"), parseInstant(required(lease, "expiresAt")), lease.optInt("attempt", 1), job.toString()));
    }

    public JSONObject submitResult(String sessionToken, JSONObject submitRequest) throws Exception {
        JSONObject response = request("POST", SUBMIT_PATH, submitRequest, Headers.session(sessionToken), 30_000);
        JSONObject payload = unwrapData(response);
        if (payload.has("accepted") && !payload.optBoolean("accepted")) throw new ApiException(409, "RESULT_REJECTED", "result was not accepted", false, null);
        return payload;
    }

    private JSONObject request(String method, String path, JSONObject body, Headers headers, int readTimeoutMs) throws Exception {
        URL url = new URL(GatewayUrlPolicy.requireControllerUrl(profile.controllerUrl) + path);
        ...
        for (java.util.Iterator<String> it = headers.values.keys(); it.hasNext();) {
            String name = it.next();
            connection.setRequestProperty(name, headers.values.optString(name));
        }
        addDeviceProof(connection, method, path, bodyBytes);
        if (bodyBytes.length > 0) {
            connection.setDoOutput(true);
            connection.setFixedLengthStreamingMode(bodyBytes.length);
            try (java.io.OutputStream out = connection.getOutputStream()) { out.write(bodyBytes); }
        }
        int status;
        byte[] responseBytes;
        try {
            ...
            responseBytes = readLimited(stream);
        } finally {
            connection.disconnect();
        }
        JSONObject response;
        try { response = responseBytes.length == 0 ? new JSONObject() : new JSONObject(new String(responseBytes, StandardCharsets.UTF_8)); }
        catch (Exception parse) { throw new ApiException(status, "INVALID_RESPONSE", "PC01 returned invalid JSON", status >= 500, null); }
        if (status < 200 || status >= 300) throw errorFromBody(status, response, status == 429 || status >= 500);
        return response;
    }
    ...
    private void addDeviceProof(HttpURLConnection connection, String method, String path, byte[] body) throws Exception {
        ...
        connection.setRequestProperty("X-TigerIQ-Device-Key-Fingerprint", deviceKey.publicKeyFingerprintSha256());
        connection.setRequestProperty("X-TigerIQ-Device-Public-Key", deviceKey.publicKeyBase64());
        connection.setRequestProperty("X-TigerIQ-Device-Timestamp", Long.toString(timestamp));
    ...
    private static ApiException errorFromBody(int status, JSONObject body, boolean defaultRetryable) {
        JSONObject error = body.optJSONObject("error");
        if (error == null) return new ApiException(status, "HTTP_" + status, "PC01 request failed", defaultRetryable, null);
        String code = error.optString("code", "HTTP_" + status), message = error.optString("message", "PC01 request failed");
        boolean retryable = error.has("retryable") ? error.optBoolean("retryable") : defaultRetryable;
        Long retryAfterMs = error.has("retryAfterMs") && !error.isNull("retryAfterMs") ? error.optLong("retryAfterMs") : null;
    ...
    private static byte[] readLimited(InputStream stream) throws Exception {
        ...
        try (InputStream input = stream; ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            while ((read = input.read(buffer)) >= 0) {
                total += read;
                if (total > MAX_RESPONSE_BYTES) throw new ApiException(502, "RESPONSE_TOO_LARGE", "PC01 response too large", false, null);
