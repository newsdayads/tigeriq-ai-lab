...
export function renderWorkforceContentV5(data: ExecutiveDashboardV4, selectedId = ''): string {
  ...
  return `<div class="ev5-shell" data-live-section="entity-content"><div class="ev5-intro"><h1>Nhân sự</h1><p>AI nào đang làm gì — chỉ nhân viên AI trong danh sách chuẩn, không trộn dịch vụ kỹ thuật.</p></div><div class="ev5-grid">${data.people.filter((person) => person.key !== 'VY').map((person) => personCard(person,data)).join('')}</div></div>`;
}
...
export function renderSystemContentV5(data: ExecutiveDashboardV4, selectedId = ''): string {
  ...
  return `<div class="ev5-shell" data-live-section="entity-content"><div class="ev5-intro"><h1>Hệ thống</h1><p>Máy có khỏe không — thiếu nguồn trực tiếp thì giữ trạng thái Chưa xác minh.</p></div><div class="ev5-grid systems">${data.systems.map(systemCard).join('')}</div></div>`;
}
