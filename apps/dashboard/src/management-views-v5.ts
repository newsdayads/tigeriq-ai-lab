...
export function renderProjectsV5(data: ExecutiveDashboardV4, selectedId = ''): string {
  ...
  const source = `<div class="mv5-source"><b>${esc(data?.sourceStatus || 'Nguồn trực tiếp')}</b><span>${esc(data.sourceNote || 'Dữ liệu đang đồng bộ từ nguồn hiện hành.')}</span></div>`;
  if (selectedId) {
    const group = groups.find((item) => item.id === selectedId);
    if (!group) return `<div class="mv5-shell" data-live-section="management-content">${source}<div class="mv5-empty">Không tìm thấy dự án trong dữ liệu hiện hành.</div></div>`;
    ...
    return `<div class="mv5-shell" data-live-section="management-content"><a class="mv5-back" href="/?view=models">← Dự án</a>${source}<div class="mv5-title"><div><small>${esc(group.id)}</small><h1>${esc(group.name)}</h1></div><span>${age(group.last)}</span></div><div class="mv5-focus"><small>Mốc đang tập trung</small><b>${next ? esc(next.title) : 'Không có việc mở'}</b><span>${next ? esc(next.currentStep || next.next) : 'Dự án không có công việc đang xử lý.'}</span></div><div class="mv5-metrics"><article><small>Đang làm</small><b>${group.active}</b></article><article><small>Bị chặn</small><b>${group.blocked}</b></article><article><small>Chờ xử lý</small><b>${group.works.filter(w=>w.tone==='waiting'||w.tone==='paused'||w.tone==='stale').length}</b></article><article><small>Hoàn tất</small><b>${group.done}</b></article></div><h3>Công việc thuộc dự án</h3><div class="mv5-list">${rows || '<div class="mv5-empty">Chưa có công việc.</div>'}</div></div>`;
  }
  ...
  return `<div class="mv5-shell" data-live-section="management-content"><div class="mv5-intro"><h1>Dự án</h1><p>Nhìn theo mục tiêu và công việc đang chạy; mỗi dự án dùng mã dự án ổn định, không dùng phần trăm giả.</p></div>${source}<div class="mv5-grid">${cards || '<div class="mv5-empty">Chưa có dự án được liên kết.</div>'}</div></div>`;
}
...
export function renderReportsV5(data: ExecutiveDashboardV4): string {
  ...
  return `<div class="mv5-shell" data-live-section="management-content"><div class="mv5-intro"><h1>Báo cáo điều hành</h1><p>Chỉ số để quyết định hôm nay, không dùng số liệu ước lượng giả.</p></div>${source}<div class="mv5-metrics mv5-metrics-five">${cards.map(([name,value,note])=>`<article><small>${esc(name)}</small><b>${esc(value)}</b><span>${esc(note)}</span></article>`).join('')}</div><div class="mv5-data-gap"><b>Thời gian xử lý / mức đáp ứng:</b><span>Chưa đủ dữ liệu chuẩn để tính trung vị, phân vị 95 hoặc tỷ lệ tự động.</span></div><div class="mv5-report-grid"><section><h2>Hoạt động gần nhất</h2>${recent || '<div class="mv5-empty">Chưa có sự kiện mới được xác minh.</div>'}</section><section><h2>Điểm cần chú ý</h2>${problems || '<div class="mv5-empty good">Không có công việc bị chặn hoặc mất tín hiệu.</div>'}</section></div></div>`;
}

export function renderSettingsV5(data?: ExecutiveDashboardV4): string {
  const rows=[['Đồng bộ trực tiếp','Bật','Luồng sự kiện trực tiếp; khi mất kết nối dùng cập nhật dự phòng 5 giây'],['Ngưỡng mất tín hiệu','120 giây','Công việc thiếu nhịp sống mới sẽ được đánh dấu mất tín hiệu'],['Nguồn công việc',data?.sourceStatus || 'Nguồn trực tiếp',data?.sourceNote || 'Đang đọc nguồn hiện hành'],['Điều khiển ghi','Khóa khi chưa xác thực','Mọi thao tác thay đổi cần phiên đăng nhập, mã chống giả mạo và nhật ký'],['Phạm vi mạng','Riêng tư','Chỉ địa chỉ máy cục bộ hoặc mạng riêng được phép phục vụ Web Control']];
  return `<div class="mv5-shell" data-live-section="management-content"><div class="mv5-intro"><h1>Cài đặt vận hành</h1><p>Những quy tắc đang tác động trực tiếp lên Trung tâm điều hành V5; các nguồn trạng thái ưu tiên đọc trước khi cho phép hành động.</p></div><div class="mv5-source"><b>${esc(data?.sourceStatus || 'Nguồn trực tiếp')}</b><span>${esc(data?.sourceNote || '')}</span></div><div class="mv5-settings">${rows.map(([name,value,note])=>`<article><div><b>${esc(name)}</b><small>${esc(note)}</small></div><span>${esc(value)}</span></article>`).join('')}</div><section class="mv5-callout"><b>Điều khiển an toàn đã có nền tảng.</b><p>Các nút thay đổi hệ thống chỉ mở khi phiên đăng nhập hợp lệ. Không hiển thị nút giả và không tự vượt qua giới hạn bảo mật.</p></section></div>`;
}
