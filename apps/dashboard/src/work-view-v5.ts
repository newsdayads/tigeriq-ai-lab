...
function card(work: ExecutiveWorkV4, url: URL): string {
  ...
  const evidence = work.evidenceRef ? `<a class="wv5-evidence" href="${esc(work.evidenceRef)}">Bằng chứng cuối</a>` : '<span class="wv5-muted">Chưa có liên kết bằng chứng</span>';
...
function detail(data: ExecutiveDashboardV4, url: URL, selectedRaw: string): string {
  ...
  if (!work) return `<section class="wv5-detail"><a class="wv5-back" href="${back}">← Công việc</a><div class="wv5-empty">Không tìm thấy ${esc(selectedId)} trong dữ liệu hiện hành.</div></section>`;
  const timeline = work.timeline?.length ? work.timeline.map((event) => `<li><time>${esc(event.timestamp ? new Date(event.timestamp).toLocaleString('vi-VN',{hour12:false}) : '—')}</time><p>${esc(event.message)}</p>${event.evidenceRef ? `<a href="${esc(event.evidenceRef)}">Mở bằng chứng</a>` : ''}</li>`).join('') : '<li class="wv5-empty">Chưa có timeline được xác minh.</li>';
  return `<section class="wv5-detail"><a class="wv5-back" href="${back}">← Công việc</a><div class="wv5-detail-head"><div><small>${esc(stableWorkIdV5(work))} · ${esc(work.priority ?? '—')}</small><h2>${esc(work.title)}</h2><p>${esc(work.goal ?? 'Chưa có mục tiêu chi tiết được chuẩn hóa.')}</p></div><span class="wv5-status ${toneClass(work)}">${esc(work.status)}</span></div><div class="wv5-detail-grid"><article><small>AI phụ trách</small><b>${esc(work.owner)}</b></article><article><small>Bước hiện tại</small><b>${esc(work.currentStep ?? 'Chưa xác minh')}</b></article><article><small>Hoạt động cuối</small><b>${esc(ageLabel(work.lastActivityAt))}</b></article><article><small>Việc kế tiếp</small><b>${esc(work.next)}</b></article></div><div class="wv5-timeline"><h3>Dòng thời gian / bằng chứng</h3><ol>${timeline}</ol></div></section>`;
}
...
.wv5-shell{min-width:0}.wv5-intro{display:flex;align-items:flex-end;justify-content:space-between;gap:16px;margin:0 0 14px}.wv5-intro h1{margin:0;font-size:22px}.wv5-intro p{margin:4px 0 0;color:#9fb2ca}.wv5-intro>span{color:#7890ac;font-size:11px}.wv5-filters{display:grid;grid-template-columns:minmax(220px,1.6fr) repeat(3,minmax(120px,.7fr)) auto auto auto;gap:8px;margin-bottom:14px}.wv5-filters input,.wv5-filters select,.wv5-filters button,.wv5-filters>a{min-width:0;height:38px;border:1px solid #28527e;border-radius:8px;background:#0a294d;color:#e9f2ff;padding:0 10px;transition:border-color .15s ease,background .15s ease,box-shadow .15s ease}.wv5-filters button{background:#0875dc;cursor:pointer}.wv5-filters>a{display:grid;place-items:center;color:#b7cae0}.wv5-check{display:flex;align-items:center;gap:6px;color:#a9bdd4;white-space:nowrap}.wv5-check input{width:auto;height:auto}.wv5-board{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;align-items:start}.wv5-lane{min-width:0;background:rgba(7,27,55,.58);border:1px solid #1d436e;border-radius:12px;overflow:hidden}.wv5-lane>header{height:47px;display:flex;align-items:center;justify-content:space-between;padding:0 12px;border-bottom:1px solid #1d436e}.wv5-lane h2{margin:0;font-size:13px}.wv5-lane header b{min-width:25px;height:25px;border-radius:999px;display:grid;place-items:center;background:#183d66;font-size:11px}.wv5-stack{display:grid;gap:9px;padding:9px}.wv5-card{min-width:0;border:1px solid #224c79;background:#0b274a;border-radius:10px;padding:10px}.wv5-card.blocked{border-color:#704052}.wv5-card.stale{opacity:.78}.wv5-card-top{display:flex;gap:8px;align-items:flex-start;justify-content:space-between}.wv5-title{min-width:0;display:grid;gap:3px;transition:transform .15s ease,color .15s ease}.wv5-title:hover{transform:translateY(-1px);color:#fff}.wv5-title b{font-size:10px;color:#58b8ff}.wv5-title span{font-size:12px;font-weight:650;line-height:1.35;overflow-wrap:anywhere;word-break:break-word}.wv5-title:focus-visible,.wv5-evidence:focus-visible,.wv5-back:focus-visible,.wv5-filters button:focus-visible,.wv5-filters>a:focus-visible{outline:2px solid #5fc4ff;outline-offset:2px}.wv5-priority{flex:none;padding:2px 6px;border-radius:5px;background:#173b62;color:#ffd477;font-size:10px;font-weight:700}.wv5-meta,.wv5-foot{display:flex;gap:8px;align-items:center;flex-wrap:wrap;color:#93abc5;font-size:10px}.wv5-meta{margin:8px 0}.wv5-step{display:grid;gap:3px;margin:8px 0;padding:8px;border-radius:7px;background:#091f3b}.wv5-step small{color:#7f99b6}.wv5-step strong{font-size:11px;font-weight:600;overflow-wrap:anywhere}.wv5-status{display:inline-flex;padding:3px 7px;border-radius:6px;background:#23364f;color:#a8bbd1;font-size:10px;font-weight:700}.wv5-status.active{background:#0c3d35;color:#36dd8c}.wv5-status.blocked{background:#4b2331;color:#ff7489}.wv5-status.done{background:#123b58;color:#52baff}.wv5-status.waiting{background:#50351c;color:#ffae58}.wv5-evidence,.wv5-back{transition:color .15s ease,background .15s ease}.wv5-evidence{margin-left:auto;color:#56baff}.wv5-evidence:hover,.wv5-back:hover{color:#bde6ff}.wv5-muted{margin-left:auto;color:#738aa5}.wv5-empty{padding:18px 10px;color:#8197b1;text-align:center;font-size:11px}.wv5-detail{border:1px solid #214a78;border-radius:12px;background:linear-gradient(180deg,rgba(11,34,65,.96),rgba(8,28,54,.97));padding:15px}.wv5-back{display:inline-block;margin-bottom:12px;color:#67bdff}.wv5-detail-head{display:flex;justify-content:space-between;gap:18px}.wv5-detail-head h2{margin:4px 0 7px}.wv5-detail-head p{margin:0;color:#a7bbd2;max-width:850px;overflow-wrap:anywhere;word-break:break-word}.wv5-detail-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin:16px 0}.wv5-detail-grid article{min-width:0;border:1px solid #244b77;border-radius:8px;background:#0a294d;padding:10px;display:grid;gap:5px}.wv5-detail-grid small{color:#839bb6}.wv5-detail-grid b{overflow-wrap:anywhere}.wv5-timeline{border-top:1px solid #214a78;padding-top:12px}.wv5-timeline h3{margin:0 0 8px}.wv5-timeline ol{list-style:none;margin:0;padding:0;display:grid;gap:8px}.wv5-timeline li{border-left:2px solid #286da6;padding:5px 9px}.wv5-timeline time{font-size:10px;color:#829ab6}.wv5-timeline p{margin:3px 0;color:#d8e6f5}.wv5-timeline a{font-size:10px;color:#59baff}
@media(max-width:1250px){.wv5-board{grid-template-columns:repeat(2,minmax(0,1fr))}.wv5-filters{grid-template-columns:repeat(3,minmax(0,1fr))}.wv5-detail-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:760px){.wv5-board,.wv5-filters,.wv5-detail-grid{grid-template-columns:minmax(0,1fr)}.wv5-intro,.wv5-detail-head{align-items:flex-start;flex-direction:column}.wv5-evidence,.wv5-muted{margin-left:0}}
@media(prefers-reduced-motion:reduce){.wv5-title,.wv5-evidence,.wv5-back,.wv5-filters input,.wv5-filters select,.wv5-filters button,.wv5-filters>a{transition:none}}
`;