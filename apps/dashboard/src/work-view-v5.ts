...
export function renderWorkContentV5(data: ExecutiveDashboardV4, url: URL, selectedRaw = ''): string {
  return `<div class="wv5-shell" data-live-section="work-v5"><div class="wv5-intro"><div><h1>Công việc</h1><p>Execution board — chỉ hiển thị trạng thái có nguồn xác minh.</p></div><span>generated ${esc(data.generatedAt)}</span></div>${selectedRaw ? detail(data,url,selectedRaw) : `${filterBar(data,url)}${board(data,url)}`}</div>`;
}
