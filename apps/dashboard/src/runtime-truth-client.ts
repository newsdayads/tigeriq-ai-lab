...
type RuntimeEmployeeTelemetry=WorkforceEmployeeTelemetry&{lastHeartbeatAt:string|null;stale:boolean};
type RuntimeTaskTelemetry=WorkforceTaskTelemetry&{
...
let latestTruth:ServerTelemetry|null=null;
...
function remember(value:ServerTelemetry):ServerTelemetry{latestTruth=value;return value;}
export function latestControllerRuntimeTruth():ServerTelemetry|null{return latestTruth?structuredClone(latestTruth):null;}
...
export function normalizeControllerRuntimeTruth(raw:unknown,baseUrl='http://127.0.0.1:8790'):ServerTelemetry{
  const root=rec(raw);if(!root||root.ok!==true||root.protocol!=='runtime-truth-v1')return remember(emptyTelemetry());
  ...
  const employeeRows=rows(root.employees),jobRows=rows(root.jobs),leases=rows(root.leases),counts=stageCounts(queue?.stageCounts);
  const leaseByJob=new Map(leases.map(row=>[text(row.jobId,160)??'',row]));
  const baseTasks=jobRows.map(row=>({
    taskId:text(row.jobId,160)??'UNKNOWN',objective:text(row.objective,512)??text(row.title,512)??'Runtime job',stage:text(row.stage,32)??'unknown',priority:text(row.priority,16)??'P1',
    assignedEmployeeId:text(leaseByJob.get(text(row.jobId,160)??'')?.employeeId,160)??text(row.targetEmployeeId,160),
    createdAt:text(row.createdAt,64),updatedAt:text(row.updatedAt,64),attempts:Math.max(0,num(row.attempts)??0),maxAttempts:Math.max(0,num(row.maxAttempts)??0),lastFailureCode:text(row.lastFailureCode,128),
  }));
  const roster:RuntimeEmployeeTelemetry[]=employeeRows.map(row=>{
    const employeeId=text(row.employeeId,160)??'UNKNOWN';
    const current=baseTasks.filter(task=>task.assignedEmployeeId===employeeId&&activeStage(task.stage)&&leaseByJob.has(task.taskId)).map(task=>task.taskId);
    const online=row.online===true,health=text(row.health,32)??'unknown',heartbeatAt=text(row.heartbeatAt,64),stale=row.stale===true;
    const roles=Array.isArray(row.roles)?row.roles.filter((value):value is string=>typeof value==='string'):[];
    return {employeeId,displayName:text(row.displayName,160)??employeeId,department:'Runtime',role:roles[0]??'worker',nodeId:text(row.deviceId,160)??'unbound',provider:null,model:null,availability:!online?'offline':health==='degraded'?'degraded':current.length?'busy':'idle',healthScore:online?100:0,concurrencyLimit:Math.max(1,num(row.concurrencyLimit)??1),activeTaskCount:current.length,currentTaskIds:current,lastHeartbeatAt:heartbeatAt,stale};
  });
  const employeeById=new Map(roster.map(row=>[row.employeeId,row]));
  const taskList:RuntimeTaskTelemetry[]=baseTasks.map(task=>{
    const lease=leaseByJob.get(task.taskId),employeeId=text(lease?.employeeId,160)??task.assignedEmployeeId??null,employee=employeeId?employeeById.get(employeeId):undefined;
    return {...task,
      leaseId:text(lease?.leaseId,160),leaseEmployeeId:employeeId,leaseDeviceId:text(lease?.deviceId,160),leaseWorkerKind:text(lease?.workerKind,64),leaseAttempt:num(lease?.attempt),leasedAt:text(lease?.leasedAt,64),leaseExpiresAt:text(lease?.expiresAt,64),
      workerHeartbeatAt:employee?.lastHeartbeatAt??null,workerOnline:employee?.availability!=='offline'&&Boolean(employee),workerStale:employee?.stale??true,sourceRef:`runtime-truth-v1:job:${task.taskId}`,
    };
  });
...
export async function collectControllerRuntimeTruth(baseUrl:string,ingressToken:string):Promise<ServerTelemetry>{
  if(!baseUrl||ingressToken.trim().length<32)return remember(emptyTelemetry());
  try{const response=await fetch(new URL('/api/v1/runtime-truth',baseUrl),{headers:{authorization:`Bearer ${ingressToken}`},cache:'no-store'});if(!response.ok)return remember(emptyTelemetry());return normalizeControllerRuntimeTruth(await response.json(),baseUrl);}catch{return remember(emptyTelemetry());}
}
