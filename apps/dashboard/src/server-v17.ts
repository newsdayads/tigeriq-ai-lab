...
export function applyNativeSelfHealStateV5(data: ExecutiveDashboardV4, state: NativeSelfHealV5): ExecutiveDashboardV4 {
  ...
  const systems = data.systems.map((row) => { const task = bind[row.key]; if (!task || !(task in tasks)) return row; const running = tasks[task] === 'Running'; return { ...row, status: running ? 'Ho\u1ea1t \u0111\u1ed9ng' : 'Kh\u00f4ng ho\u1ea1t \u0111\u1ed9ng', tone: running ? 'active' : 'blocked', note: `${running ? 'Đang chạy' : 'Không chạy'} · tác vụ hệ thống: ${task}` } as ExecutiveSystemV4; });
