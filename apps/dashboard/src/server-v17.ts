...
import { ENTITY_V5_CSS, renderSystemContentV5, renderWorkforceContentV5 } from './entity-views-v5.js';
...
function sidebar(active: View): string {
  return `<aside class="x-sidebar">${brand()}<nav class="x-nav">${primaryNav(active)}</nav><div class="x-side-bottom"><em>Smarter People<br>Bigger Impact</em><span>TigerIQ AI Lab<br>Web Control V5</span></div></aside>`;
}

function header(title = 'TigerIQ AI Lab', subtitle = 'Bảng điều hành', live = false): string {
  return `<header class="x-header"><div class="x-title"><h1>${esc(title)}</h1><p>${esc(subtitle)}</p></div><div class="x-header-tools"><form class="x-search" method="get" action="/"><input type="hidden" name="view" value="work"><span>${svg(icons.search)}</span><input name="q" placeholder="Tìm kiếm công việc, người phụ trách…" aria-label="Tìm kiếm"></form>${live ? `<div class="x-live" id="x-live"><i></i><span id="x-live-state">V5 · đang kết nối</span><small id="x-live-age">đang đồng bộ</small></div><button class="x-iconbtn" id="x-refresh" type="button" title="Cập nhật ngay">${svg(icons.refresh)}</button>` : ''}<div class="x-owner"><b>AS</b><span><strong>Anh Sơn</strong><small>Owner</small></span></div></div></header>`;
}
...
export function renderExecutiveWorkforceV5(data: ExecutiveDashboardV4, selectedId = ''): string {
  return `<!doctype html><html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>TigerIQ — Nhân sự V5</title><style>${BASE_CSS}${ENTITY_V5_CSS}</style></head><body><div class="x-app" data-version="${WEB_LOCAL_VERSION_V17}" data-view="workforce-v5">${sidebar('workforce')}${header('TigerIQ AI Lab','Nhân sự · AI employee V5',true)}<main class="x-main">${renderWorkforceContentV5(data,selectedId)}</main></div>${liveScript()}</body></html>`;
}

export function renderExecutiveSystemV5(data: ExecutiveDashboardV4, selectedId = ''): string {
  return `<!doctype html><html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>TigerIQ — Hệ thống V5</title><style>${BASE_CSS}${ENTITY_V5_CSS}</style></head><body><div class="x-app" data-version="${WEB_LOCAL_VERSION_V17}" data-view="system-v5">${sidebar('system')}${header('TigerIQ AI Lab','Hệ thống · health V5',true)}<main class="x-main">${renderSystemContentV5(data,selectedId)}</main></div>${liveScript()}</body></html>`;
}
...
export function renderExecutiveManagementV5(data: ExecutiveDashboardV4, view: 'models'|'reports'|'settings', selectedId = ''): string {
  ...
  return `<!doctype html><html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>TigerIQ — ${title} V5</title><style>${BASE_CSS}${MANAGEMENT_V5_CSS}${OVERVIEW_SIGNALS_V5_CSS}</style></head><body><div class="x-app" data-version="${WEB_LOCAL_VERSION_V17}" data-view="${view}-v5">${sidebar(view)}${header('TigerIQ AI Lab',`${title} · Web Control V5`,true)}<main class="x-main">${content}</main></div>${liveScript()}</body></html>`;
}
