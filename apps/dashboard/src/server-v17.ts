...
function viewFrom(urlValue: string | undefined): View {
  ...
  if (url.pathname === '/people' || url.pathname.startsWith('/people/')) return 'workforce';
  if (url.pathname === '/system' || url.pathname.startsWith('/system/')) return 'system';
  if (url.pathname === '/projects' || url.pathname.startsWith('/projects/')) return 'models';
  const value = url.searchParams.get('view') ?? 'work';
...
function toneClass(tone: string): string {
  return ['active', 'waiting', 'blocked', 'done', 'paused', 'stale', 'unknown'].includes(tone) ? tone : 'unknown';
}
...
function header(title = 'TigerIQ AI Lab', subtitle = 'Bảng điều hành', live = false): string {
  return `<header class="x-header"><div class="x-title"><h1>${esc(title)}</h1><p>${esc(subtitle)}</p></div><div class="x-header-tools"><form class="x-search" method="get" action="/"><input type="hidden" name="view" value="work"><span>${svg(icons.search)}</span><input name="q" placeholder="Tìm kiếm công việc, người phụ trách…" aria-label="Tìm kiếm"></form>${live ? `<div class="x-live" id="x-live"><i></i><span id="x-live-state">Đang kết nối dữ liệu</span><small id="x-live-age">đang đồng bộ</small></div><button class="x-iconbtn" id="x-refresh" type="button" title="Cập nhật ngay">${svg(icons.refresh)}</button>` : ''}<div class="x-owner"><b>AS</b><span><strong>Anh Sơn</strong><small>Quản trị</small></span></div></div></header>`;
}
...
function team(data: ExecutiveDashboardV4): string {
  ...
  return `<section class="x-card x-team" data-live-section="team"><div class="x-card-head"><h2>${svg(icons.users)} Đội AI</h2><a href="/?view=workforce">Xem chi tiết →</a></div><div class="x-team-grid">${data.people.map((person, index) => `<a class="x-team-link" href="/people/${encodeURIComponent(person.key)}"><article><div class="x-avatar ${tones[index]}">${esc(person.initials)}</div><h3>${esc(person.name)}</h3><span class="x-status ${toneClass(person.tone)}"><i></i>${esc(person.status)}</span><p>${esc(person.current)}</p></article></a>`).join('')}</div></section>`;
}
...
function systems(data: ExecutiveDashboardV4): string {
  return `<section class="x-card x-systems" data-live-section="systems"><div class="x-card-head"><h2>${svg(icons.settings)} Hệ thống</h2><a href="/?view=system">Xem chi tiết →</a></div><div class="x-system-grid">${data.systems.slice(0, 4).map((row) => `<a class="x-system-link" href="/system/${encodeURIComponent(row.key)}"><article><span class="x-system-icon ${toneClass(row.tone)}">${systemIcon(row)}</span><div><h3>${esc(row.name)}</h3><span class="x-status ${toneClass(row.tone)}"><i></i>${esc(row.status)}</span><p>${esc(row.note)}</p></div></article></a>`).join('')}</div></section>`;
}
...
export function renderExecutiveOverviewV4(data: ExecutiveDashboardV4): string {
  return `<!doctype html><html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>TigerIQ — Trung tâm điều hành</title><style id="x-executive-v5-command">${BASE_CSS}${UI_HARDENING_CSS}${COMMAND_OVERVIEW_V5_CSS}${COMMAND_OVERVIEW_V5_CSS_MORE}${COMMAND_OVERVIEW_V5_CSS_LAST}</style></head><body><div class="x-app" data-version="${WEB_LOCAL_VERSION_V17}" data-layout="command-center-v5" data-font="segoe-ui">${sidebar('overview')}${header('TigerIQ AI Lab','Trung tâm điều hành trực tiếp',true)}<main class="x-main">${renderCommandOverviewV5(data)}</main></div>${liveScript()}</body></html>`;
}

export function renderExecutiveWorkV5(data: ExecutiveDashboardV4, url: URL, selectedRaw = ''): string {
  return `<!doctype html><html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>TigerIQ — Công việc V5</title><style id="x-executive-v5-work">${BASE_CSS}${UI_HARDENING_CSS}${WORK_V5_CSS}</style></head><body><div class="x-app" data-version="${WEB_LOCAL_VERSION_V17}" data-view="work-v5">${sidebar('work')}${header('TigerIQ AI Lab','Công việc · bảng thực thi trực tiếp',true)}<main class="x-main">${renderWorkContentV5(data,url,selectedRaw)}</main></div>${liveScript()}</body></html>`;
}

export function renderExecutiveWorkforceV5(data: ExecutiveDashboardV4, selectedId = ''): string {
  return `<!doctype html><html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>TigerIQ — Nhân sự V5</title><style>${BASE_CSS}${UI_HARDENING_CSS}${ENTITY_V5_CSS}${ENTITY_V5_CSS_EXTRA}</style></head><body><div class="x-app" data-version="${WEB_LOCAL_VERSION_V17}" data-view="workforce-v5">${sidebar('workforce')}${header('TigerIQ AI Lab','Nhân sự · ai đang làm gì',true)}<main class="x-main">${renderWorkforceContentV5(data,selectedId)}</main></div>${liveScript()}</body></html>`;
}

export function renderExecutiveSystemV5(data: ExecutiveDashboardV4, selectedId = ''): string {
  return `<!doctype html><html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>TigerIQ — Hệ thống V5</title><style>${BASE_CSS}${UI_HARDENING_CSS}${ENTITY_V5_CSS}${ENTITY_V5_CSS_EXTRA}</style></head><body><div class="x-app" data-version="${WEB_LOCAL_VERSION_V17}" data-view="system-v5">${sidebar('system')}${header('TigerIQ AI Lab','Hệ thống · sức khỏe trực tiếp',true)}<main class="x-main">${renderSystemContentV5(data,selectedId)}</main></div>${liveScript()}</body></html>`;
}
...
export function renderExecutiveManagementV5(data: ExecutiveDashboardV4, view: 'models'|'reports'|'settings', selectedId = ''): string {
  ...
  return `<!doctype html><html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>TigerIQ — ${title} V5</title><style>${BASE_CSS}${UI_HARDENING_CSS}${MANAGEMENT_V5_CSS}${OVERVIEW_SIGNALS_V5_CSS}</style></head><body><div class="x-app" data-version="${WEB_LOCAL_VERSION_V17}" data-view="${view}-v5">${sidebar(view)}${header('TigerIQ AI Lab',`${title} · trung tâm quản trị`,true)}<main class="x-main">${content}</main></div>${liveScript()}</body></html>`;
}
...
export function themeFunctionalPageV4(input: string, view: View): string {
  ...
  const css = `<style id="x-functional-v4">${BASE_CSS}${UI_HARDENING_CSS}
...
export function applyNativeSelfHealStateV5(data: ExecutiveDashboardV4, state: NativeSelfHealV5): ExecutiveDashboardV4 {
  ...
  const bind: Record<string,string[]> = { worker:['TigerIQ PC01 Native Worker'], control:['TigerIQ Workforce Controller'], planner:['TigerIQ Autonomous Planner'], orchestrator:['TigerIQ Mission Orchestrator'], supervisor:['TigerIQ Autonomy Supervisor V2','TigerIQ Autonomy Supervisor'], remote:['TigerIQ Desktop Commander Remote'], ollama:['TigerIQ Ollama Runtime'] };
  const systems = data.systems.map((row) => { const task = bind[row.key]?.find((name) => name in tasks); if (!task) return row; const running = tasks[task] === 'Running'; return { ...row, status: running ? 'Ho\u1ea1t \u0111\u1ed9ng' : 'Kh\u00f4ng ho\u1ea1t \u0111\u1ed9ng', tone: running ? 'active' : 'blocked', note: `${running ? 'Đang chạy' : 'Không chạy'} · tác vụ hệ thống: ${task}` } as ExecutiveSystemV4; });
...
async function relay(options: OwnerCockpitV17Options, req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (req.method === 'GET' && (path === '/projects' || path.startsWith('/projects/') || (path === '/' && view === 'models'))) {
    const data = await loadOverviewData(options);
    const selectedId = path.startsWith('/projects/') ? decodeURIComponent(path.slice('/projects/'.length)) : '';
    ...
    res.end(renderExecutiveManagementV5(data, 'models', selectedId)); return;
  }
  if (req.method === 'GET' && path === '/' && (view === 'reports' || view === 'settings')) {
    const data = await loadOverviewData(options);
    res.statusCode = 200; res.setHeader('content-type','text/html; charset=utf-8'); res.setHeader('cache-control','no-store');
    res.setHeader('content-security-policy', "default-src 'none'; style-src 'unsafe-inline'; script-src 'unsafe-inline'; img-src 'self' data:; connect-src 'self'; base-uri 'none'; frame-ancestors 'none'; form-action 'self'");
    res.end(renderExecutiveManagementV5(data, view)); return;
  }
  if (req.method === 'GET' && (path === '/people' || path.startsWith('/people/') || (path === '/' && view === 'workforce'))) {
    const data = await loadOverviewData(options);
    const selectedId = path.startsWith('/people/') ? decodeURIComponent(path.slice('/people/'.length)) : '';
    res.statusCode = 200; res.setHeader('content-type','text/html; charset=utf-8'); res.setHeader('cache-control','no-store');
    res.setHeader('content-security-policy', "default-src 'none'; style-src 'unsafe-inline'; script-src 'unsafe-inline'; img-src 'self' data:; connect-src 'self'; base-uri 'none'; frame-ancestors 'none'; form-action 'self'");
    res.end(renderExecutiveWorkforceV5(data, selectedId)); return;
  }
  if (req.method === 'GET' && (path === '/system' || path.startsWith('/system/') || (path === '/' && view === 'system'))) {
    const data = await loadOverviewData(options);
    const selectedId = path.startsWith('/system/') ? decodeURIComponent(path.slice('/system/'.length)) : '';
    res.statusCode = 200; res.setHeader('content-type','text/html; charset=utf-8'); res.setHeader('cache-control','no-store');
    res.setHeader('content-security-policy', "default-src 'none'; style-src 'unsafe-inline'; script-src 'unsafe-inline'; img-src 'self' data:; connect-src 'self'; base-uri 'none'; frame-ancestors 'none'; form-action 'self'");
    res.end(renderExecutiveSystemV5(data, selectedId)); return;
  }
...
function writeLiveEvent(res: ServerResponse, event: LiveEventV5): void {
...
export async function startOwnerCockpitV17(options: OwnerCockpitV17Options) {
  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/api/events') {
        ...
        res.writeHead(200, { 'content-type': 'text/event-stream; charset=utf-8', 'cache-control': 'no-store', connection: 'keep-alive', 'x-content-type-options': 'nosniff', 'x-accel-buffering': 'no' });
        const lastEventId = String(req.headers['last-event-id'] ?? url.searchParams.get('lastEventId') ?? '');
        for (const event of buffer.since(lastEventId || undefined)) writeLiveEvent(res, event);
        clients.add(res);
        req.on('close', () => clients.delete(res));
        return;
      }
      await relay(options, req, res);
    } catch (error) {
      res.statusCode = 503;
  ...
  return { url: `http://${address.address}:${address.port}`, close: () => new Promise<void>((resolve, reject) => { clearInterval(pollTimer); clearInterval(heartbeatTimer); for (const client of clients) client.end(); clients.clear(); server.close((error) => error ? reject(error) : resolve()); }) };
}
