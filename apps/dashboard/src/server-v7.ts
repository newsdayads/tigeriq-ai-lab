...
export const WEB_LOCAL_VERSION = 'WEB-LOCAL-390-V3';
...
export interface OwnerCockpitV7Options {
  cockpitUrl: string;
  backendUrl: string;
  repo: string;
  host?: string;
  port?: number;
  governance?: () => Promise<GovernanceV7>;
}
...
export function resolvePriorityIssueNumber(central: Issue | null, comments: Comment[] = []): number | null {
  const newest = [...comments].sort((a, b) => commentTime(b) - commentTime(a));
  for (const comment of newest) {
    const text = String(comment.body ?? '');
    const current = text.match(/ƯU TIÊN HIỆN HÀNH\s*:[\s\S]{0,700}?#(\d+)/i);
...
async function defaultGovernance(repo: string): Promise<GovernanceV7> {
  const [central, centralComments, registry, sha] = await Promise.all([
  ...
  const currentNumber = resolvePriorityIssueNumber(central, Array.isArray(centralComments) ? centralComments : []);
  if (!currentNumber) return { central, centralComments: Array.isArray(centralComments) ? centralComments : [], registry, currentIssue: null, currentComments: [], installedSha: sha };
  const [currentIssue, currentComments] = await Promise.all([
    ghJson<Issue>(repo, `issues/${currentNumber}`).catch(() => null),
    ghJson<Comment[]>(repo, `issues/${currentNumber}/comments?per_page=100`).catch(() => []),
  ]);
  return {
    central,
    centralComments: Array.isArray(centralComments) ? centralComments : [],
    registry,
    currentIssue,
...
function latestLifecycle(comments: Comment[]): { state: string; css: string; note: string; active: boolean } {
  ...
  for (const comment of newest) {
    ...
    if (/TIGERIQ_(?:JOB|COMMAND|PC01)_CLAIMED|TIGERIQ_JOB_PROGRESS|heartbeat/i.test(text)) return { state: 'ĐANG XỬ LÝ', css: 'run', note, active: true };
  }
  return { state: 'CHỜ VIỆC', css: 'neutral', note: 'Chưa có claim/heartbeat mới', active: false };
}

function ownerCode(issue: Issue | null, comments: Comment[]): 'NV01' | 'NV02' | 'NV03' | null {
  const text = `${issue?.title ?? ''}\n${issue?.body ?? ''}\n${comments.map((item) => item.body ?? '').join('\n')}`;
  if (/NV01|NV-EXEC-01|Minh\s*\(/i.test(text)) return 'NV01';
  if (/NV02|NV-OPS-01|Khoa\s*\(/i.test(text)) return 'NV02';
  if (/NV03|NV-SYS-01|Huy\s*\(/i.test(text)) return 'NV03';
  return null;
}

function ownerName(code: 'NV01' | 'NV02' | 'NV03' | null): string {
...
function nv03Paused(governance: GovernanceV7): boolean {
...
export function controlPlaneState(telemetry: ServerTelemetry): { label: string; css: string; note: string } {
  if (!telemetry.available || telemetry.controller === null) return { label: 'CHƯA XÁC MINH', css: 'neutral', note: 'Chưa có telemetry Controller' };
...
function serverState(telemetry: ServerTelemetry): { label: string; css: string; note: string } {
  if (!telemetry.available) return { label: 'CHƯA XÁC MINH', css: 'neutral', note: 'Không có host telemetry' };
  ...
  return { label: 'TRỰC TUYẾN', css: 'ok', note: note || 'Host telemetry có phản hồi' };
}

function systemCards(telemetry: ServerTelemetry, governance: GovernanceV7): string {
  const server = serverState(telemetry);
  const control = controlPlaneState(telemetry);
  const ai = nv03Paused(governance)
    ? { label: 'TẠM NGƯNG', css: 'neutral', note: 'Huy/NV03 tạm ngưng theo Registry #335; Ollama online không đồng nghĩa AI đang làm việc' }
    : telemetry.ollama?.online
      ? { label: 'SẴN SÀNG', css: 'neutral', note: `${telemetry.ollama.models.length} model khả dụng; chưa suy diễn active job` }
      : { label: 'CHƯA XÁC MINH', css: 'neutral', note: 'Không có telemetry AI PC01' };
  return [
    ['PC01 SERVER', server],
...
function employeeCards(governance: GovernanceV7): string {
  const lifecycle = latestLifecycle(governance.currentComments);
  const owner = ownerCode(governance.currentIssue, governance.currentComments);
  const paused = nv03Paused(governance);
  const rows: Array<[string, string, string, string, string]> = [
    ['Vy (Trợ lý)', 'Điều phối / Chief of Staff', 'ĐIỀU PHỐI', 'neutral', 'Điều phối, không chiếm ownership executor'],
    ['Minh (NV01 — Thực thi trực tiếp)', 'Command 1 · Owner foreground', owner === 'NV01' && lifecycle.active ? `ĐANG XỬ LÝ #${governance.currentIssue?.number}` : 'CHỜ VIỆC', owner === 'NV01' && lifecycle.active ? 'run' : 'neutral', owner === 'NV01' && lifecycle.active ? lifecycle.note : 'Không có claim active được xác minh'],
    ['Khoa (NV02 — Vận hành tự động)', 'Command 2 · Autonomous P0-first', owner === 'NV02' && lifecycle.active ? `ĐANG XỬ LÝ #${governance.currentIssue?.number}` : 'CHỜ VIỆC', owner === 'NV02' && lifecycle.active ? 'run' : 'neutral', owner === 'NV02' && lifecycle.active ? lifecycle.note : 'Không có claim active được xác minh'],
    ['Huy (NV03 — AI PC01 / Kỹ sư Hệ thống Local)', 'Command 3 · pc01-local', paused ? 'TẠM NGƯNG' : owner === 'NV03' && lifecycle.active ? `ĐANG XỬ LÝ #${governance.currentIssue?.number}` : 'CHỜ VIỆC', paused ? 'neutral' : owner === 'NV03' && lifecycle.active ? 'run' : 'neutral', paused ? 'Theo Registry #335 / chỉ đạo Owner' : owner === 'NV03' && lifecycle.active ? lifecycle.note : 'Không có claim active được xác minh'],
  ];
...
function ownerAction(governance: GovernanceV7): string {
  const newest = [...governance.currentComments].sort((a, b) => commentTime(b) - commentTime(a));
  const explicit = newest.map((item) => String(item.body ?? '')).find((text) => /OWNER_ACTION_REQUIRED=true|CẦN ANH SƠN\s*:/i.test(text));
...
function managementSummary(governance: GovernanceV7): { number: number | null; title: string; status: string; css: string; owner: string; progress: string; current: string; next: string; updated: string } {
  const issue = governance.currentIssue;
  const lifecycle = latestLifecycle(governance.currentComments);
  const owner = ownerName(ownerCode(issue, governance.currentComments));
  if (!issue) return { number: null, title: 'Chưa resolve được P0 hiện hành từ CENTRAL #280', status: 'CHƯA XÁC MINH', css: 'neutral', owner, progress: '—', current: 'Không suy đoán trạng thái', next: 'Kiểm tra CENTRAL #280 / Registry #335', updated: '—' };
  const closed = issue.state === 'closed';
  const status = closed ? 'HOÀN TẤT — CHỜ CENTRAL CHUYỂN P0' : lifecycle.state;
  const css = closed ? 'ok' : lifecycle.css;
  const progress = closed ? '100%' : lifecycle.active ? 'Đang xử lý' : 'Chờ claim';
  const current = closed ? 'Issue đã đóng; không tiếp tục chiếu như P0 đang chạy' : lifecycle.note;
  const acceptance = section(String(issue.body ?? ''), 'Điều kiện ĐẠT').split(/\r?\n/).map((line) => line.replace(/^[-*\d.\s]+/, '').trim()).find(Boolean);
  const next = closed ? 'CENTRAL #280 chọn P0 kế tiếp' : compact(acceptance || `Hoàn thành acceptance #${issue.number} → ghi evidence → cập nhật CENTRAL`, 150);
...
export function renderManagementPanel(telemetry: ServerTelemetry, governance: GovernanceV7, now = new Date()): string {
...
async function proxyResponse(options: OwnerCockpitV7Options, req: IncomingMessage, res: ServerResponse, governanceReader: () => Promise<GovernanceV7>): Promise<void> {
  ...
  if (req.method === 'GET' && new URL(req.url ?? '/', 'http://local').pathname === '/' && upstream.ok && upstreamType.includes('text/html')) {
    ...
    const page = injectManagementPanel(html, renderManagementPanel(telemetry, governance), telemetry);
...
export async function startOwnerCockpitV7(options: OwnerCockpitV7Options) {
  ...
  const server = createServer(async (req, res) => {
    try { await proxyResponse(options, req, res, governanceReader); }
      ...
      res.setHeader('cache-control', 'no-store');
      res.end(JSON.stringify({ error: 'owner_cockpit_v7_unavailable', detail: String(error instanceof Error ? error.message : error).slice(0, 160) }));
    }
  });
