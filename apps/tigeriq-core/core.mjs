      ...
      for (let attempt = 0; attempt <= maxWatsonxRetries; attempt++) {
        try {
          ...
          if (text === undefined || text === null || String(text).trim() === '') {
            const e = new Error(attempt < maxWatsonxRetries ? 'WATSONX_TRANSIENT_EMPTY' : 'EMPTY_RESPONSE');
            e.kind = 'invalid_response';
            lastWatsonxError = e;
            if (attempt < maxWatsonxRetries) {
          ...
          lastWatsonxError = err;
          if (err?.kind === 'invalid_response' && err.message === 'WATSONX_TRANSIENT_EMPTY' && attempt < maxWatsonxRetries) {
            await sleep(400 * (attempt + 1));
            continue;
          }
          if (attempt >= maxWatsonxRetries) throw err;
        }
      }
      const e = new Error('EMPTY_RESPONSE');
      e.kind = 'invalid_response';
      throw lastWatsonxError || e;
    }
