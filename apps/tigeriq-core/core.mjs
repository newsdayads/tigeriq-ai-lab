...
import { normalizeCampaignPhases, currentCampaignGoal, campaignTransition, makePhaseCheckpoint, campaignNeedsEvidence } from './campaign-runner.mjs';
...
async function managerTick() {
  ...
  try {
    ...
    if(campaignNeedsEvidence({status:decision.status,phases,doneJobs})){
      const phase=phases[currentPhase]; const id=`JOB-${randomUUID()}`;
      ...
      await pool.query('insert into tigeriq_jobs(id,objective_id,title,prompt,capability,phase_index) values($1,$2,$3,$4,$5,$6)',[id,o.id,`Phase ${currentPhase+1} evidence: ${phase.title}`.slice(0,200),evidencePrompt.slice(0,12000),'reasoning',currentPhase]);
