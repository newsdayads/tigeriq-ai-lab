...
// Dispatch channels: event-driven, persistent to avoid CMD/PowerShell process creation
class PersistChannel {
  constructor(pool, channel, table) {
    this.pool = pool;
    this.channel = channel;
    this.table = table;
  }
  async publish(payload, timeoutMs = 5000) {
    const c = new AbortController();
    const t = setTimeout(() => c.abort(), timeoutMs);
    try {
      const res = await this.pool.query(
        `INSERT INTO ${this.table} (payload) values ($1)`,
        [JSON.stringify(payload)]
      );
      return res;
    } finally { clearTimeout(t); }
  }
  async listen(listener, stopSignal) {
    while (!stopSignal.aborted) {
      try {
        const res = await this.pool.query(`SELECT payload FROM ${this.table} ORDER BY id DESC LIMIT 1`);
        if (res.rows.length > 0) await listener(res.rows[0]);
...
function measureLatency(id, state) {
  ...
  if (state === 'QUEUED') latencyStarts.set(id, now);
    ...
    const start = latencyStarts.get(id);
    if (start != null) {
      const diff = now - start;
      latencyStarts.delete(id);
      return diff;
    }
  }
  return null;
...
async function markCampaignDone(id, success, reason) {
  const ms = measureLatency(id, 'DONE');
  const status = success ? 'DONE' : 'FAILED';
  const evt = { id, status, reason, latencyMs: ms, ts: nowIso() };
  await persistChannel.publish(evt);
  return evt;
}
    ...
    if (auth && auth.includes(TOKEN)) {
      ...
      req.on('end', async () => {
        try {
          ...
          if (payload.cmd === 'campaign') {
            const evt = await dispatchCampaign(payload);
