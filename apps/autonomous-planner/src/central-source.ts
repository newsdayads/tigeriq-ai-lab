...
export type CentralPriority='P0'|'P1'|'P2'|'P3';
export type PriorityRef={rank:number;issueNumber:number;description:string;priority:CentralPriority};
...
function githubHeaders(token:string):Record<string,string>{
  return {'accept':'application/vnd.github+json','user-agent':'TigerIQ-Central-Queue/2.0',...(token?{authorization:`Bearer ${token}`}:{})};
}
...
function parsePriority(description:string):CentralPriority{
  const explicit=/^\s*\[(P[0-3])\]\s*/i.exec(description)?.[1]?.toUpperCase();
  if(explicit==='P0'||explicit==='P1'||explicit==='P2'||explicit==='P3')return explicit;
  const legacy=/\b(P[0-3])\b/i.exec(description)?.[1]?.toUpperCase();
  if(legacy==='P0'||legacy==='P1'||legacy==='P2'||legacy==='P3')return legacy;
  return 'P0';
}
export function parseCentralPriorities(body:string):PriorityRef[]{
  ...
  for(const line of section.split(/\r?\n/)){
    ...
    if(match){
      ...
      refs.push({rank:Number(match[1]),issueNumber:Number(match[2]),description,priority:parsePriority(description)});
    }
  }
...
function sourcePrompt(issue:GithubIssue,centralVersion:number,priority:PriorityRef):string{
  ...
  return `TigerIQ CENTRAL v${centralVersion} has selected ${priority.priority} priority #${issue.number}: ${issue.title}. `+
    `Logical owner is NV02 by current CENTRAL/Registry unless a live foreign lease exists; you are an AI resource, not the owner. `+
...
export function buildCentralTask(issue:GithubIssue,centralVersion:number,priority:PriorityRef):BacklogTask{
  ...
  return {
    taskId:`CENTRAL-${level}-${issue.number}-V${centralVersion}`,
    title:`CENTRAL ${level} #${issue.number}: ${issue.title}`,
    objective:`Materialize CENTRAL ${level} priority #${issue.number} into the canonical execution queue without manual chat dispatch.`,
    status:'pending',priority:level,route:'ai_auto',actionClass:'LOCAL_AI',
    payload:{prompt:sourcePrompt(issue,centralVersion,priority),kind:'analysis',risk:'low',sourceIssueNumber:issue.number,sourceIssueUrl:issue.html_url,centralVersion,priorityRank:priority.rank,logicalOwner:'NV02',providerPolicy:{zeroCostOnly:true}},
    requiredCapabilities:['ai_resource','evidence'],requiredPermissions:['local_ai:execute','evidence:write'],expectedEvidence:['json'],
    scopeKeys:[`central/${level.toLowerCase()}/issue-${issue.number}/v${centralVersion}`],dependencies:[],requiresAuthorization:false,enabled:true,maxAttempts:3,
  };
}

export function shouldAdvanceCompletedTopPriority(_priority:PriorityRef,task:BacklogTask,state:PlannerRuntimeState):boolean{
  return state.tasks[task.taskId]?.stage==='done';
}
...
export async function reportCompletedCentralTasks(backlog:PlannerBacklog,state:PlannerRuntimeState,config:CentralSourceConfig=centralDefaults):Promise<{reported:number;jobIds:string[]}>{
  ...
  for(const task of backlog.tasks){
    if(!task.taskId.startsWith('CENTRAL-'))continue;
