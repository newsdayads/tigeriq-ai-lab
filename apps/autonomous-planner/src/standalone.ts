...
export async function plannerCycle():Promise<void>{
  const backlog=parseBacklog(await readJson(backlogPath));let state=reconcile(backlog,await loadState());await syncDispatched(state);
  const now=new Date().toISOString();for(const id of waitingDependencies(backlog,state))state.tasks[id]={stage:'waiting_dependency',updatedAt:now,reason:'dependency_not_done'};
  const selected=actionable(backlog,state,dispatchLimit);
  for(const task of selected){
    try{const created=await request('POST','/api/v1/work-orders',toControllerBody(task));const jobId=String(created?.workOrder?.jobId??'');if(!jobId)throw new Error('CONTROLLER_JOB_ID_MISSING');state.tasks[task.taskId]={stage:'dispatched',controllerJobId:jobId,updatedAt:new Date().toISOString()};console.log(JSON.stringify({event:'AUTONOMY_DISPATCH',taskId:task.taskId,jobId,priority:task.priority,route:task.route}));}
    catch(error){state.tasks[task.taskId]={stage:'failed',updatedAt:new Date().toISOString(),reason:String(error).slice(0,1024)};console.error(JSON.stringify({event:'AUTONOMY_DISPATCH_ERROR',taskId:task.taskId,message:String(error)}));}
  }
  state.lastCycleAt=new Date().toISOString();await writeJson(statePath,state);
  const held=Object.entries(state.tasks).filter(([,v])=>v.stage==='held_authorization').map(([id])=>id);
  console.log(JSON.stringify({event:'AUTONOMY_CYCLE',workspace,selected:selected.map(t=>t.taskId),heldAuthorization:held,statePath}));
}
...
export async function startPlanner():Promise<void>{
  ...
  console.log(JSON.stringify({event:'AUTONOMOUS_PLANNER_V1_START',intervalMs,dispatchLimit,backlogPath,statePath}));
