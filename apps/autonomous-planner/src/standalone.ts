...
let stopped=false;
const logPath=(process.env.TIGERIQ_AUTONOMY_LOG??'D:\\TigerIQ\\Logs\\autonomous-planner-v1.log').trim();
...
async function syncDispatched(state:PlannerRuntimeState):Promise<void>{
  for(const [taskId,item] of Object.entries(state.tasks)){
    ...
    }catch(error){runtimeLogError(JSON.stringify({event:'AUTONOMY_JOB_SYNC_ERROR',taskId,message:String(error)}));}
  }
}
...
export async function plannerCycle():Promise<void>{
  ...
  for(const task of selected){
    try{const created=await request('POST','/api/v1/work-orders',toControllerBody(task));const jobId=String(created?.workOrder?.jobId??'');if(!jobId)throw new Error('CONTROLLER_JOB_ID_MISSING');state.tasks[task.taskId]={...state.tasks[task.taskId],stage:'dispatched',controllerJobId:jobId,updatedAt:new Date().toISOString()};runtimeLog(JSON.stringify({event:'AUTONOMY_DISPATCH',taskId:task.taskId,jobId,priority:task.priority,route:task.route,policy:state.tasks[task.taskId].policy}));}
...
export async function startPlanner():Promise<void>{
  ...
  runtimeLog(JSON.stringify({event:'AUTONOMOUS_PLANNER_V1_START',intervalMs,dispatchLimit,backlogPath,statePath,authorizationPath}));
  while(!stopped){try{await plannerCycle();}catch(error){runtimeLogError(JSON.stringify({event:'AUTONOMY_CYCLE_FATAL',message:String(error)}));}for(let elapsed=0;elapsed<intervalMs&&!stopped;elapsed+=1000)await new Promise(r=>setTimeout(r,Math.min(1000,intervalMs-elapsed)));}
}
function stop(signal:string){stopped=true;runtimeLog(JSON.stringify({event:'AUTONOMOUS_PLANNER_V1_STOP',signal}));}
...
if(invokedAsMain)startPlanner().catch(error=>{runtimeLogError(JSON.stringify({event:'AUTONOMOUS_PLANNER_V1_FATAL',message:error instanceof Error?error.message:String(error)}));process.exit(1);});
