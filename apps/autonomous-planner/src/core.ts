...
export type PlannerRoute='local_ai'|'groq'|'ai_auto'|'tool'|'deterministic';
...
export function parseBacklog(raw:unknown):PlannerBacklog{
  ...
  const tasks=root.tasks.map((value,index)=>{
    ...
    const route=text(row.route,'route',32) as PlannerRoute;if(!['local_ai','groq','ai_auto','tool','deterministic'].includes(route))throw new Error('INVALID_ROUTE');
...
function validatePayload(task:BacklogTask):void{
  if(task.route==='local_ai'||task.route==='groq'||task.route==='ai_auto'){
...
export function toControllerBody(task:BacklogTask):Record<string,unknown>{
  const routeCapabilities=task.route==='groq'?['groq','evidence']:task.route==='ai_auto'?['ai_resource','evidence']:task.route==='local_ai'?['local_ai','evidence']:task.route==='tool'?['filesystem','evidence']:['evidence'];
  const routePermissions=task.route==='groq'?['cloud_ai:execute','evidence:write']:(task.route==='local_ai'||task.route==='ai_auto')?['local_ai:execute','evidence:write']:task.route==='tool'?['workspace:read','workspace:write','evidence:write']:['evidence:write'];
