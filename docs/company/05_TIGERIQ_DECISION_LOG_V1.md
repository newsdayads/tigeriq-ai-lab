...
- 2026-09-03: Owner reporting triggers `báo cáo`, `bc`, and `tiến độ` must render the standardized one-screen TigerIQ dashboard: total progress, main workstreams, P0 blocker, current work, AI workforce status, and next milestone. Legacy bullet reporting is not acceptable for these triggers.
- 2026-09-03: Owner-facing prompt requests (`prompt`, `promt`, `đưa prompt`, `qua Work`, `giao NV`) must return exactly one copyable prompt beginning `LÀM — NO YAPPING.` with sufficient context and DONE criteria for continuous execution. Repeated Work handoff retries are prohibited when unavailable/rejected; provide the single fresh-Work prompt instead and batch related work to conserve quota.
- 2026-09-03: A behavioral rule is not considered operationally DONE merely because it is documented. Runtime/application and a fresh-context acceptance test are required before claiming cross-chat enforcement.
