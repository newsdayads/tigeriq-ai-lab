...
## Audit thực tế
TigerIQ đã có Work Order cơ bản, Task Packet, hàng đợi chống trùng, scheduler theo năng lực, retry/reassignment, lease/restart recovery, Workforce Controller, FileJournal hash-chain, evidence contracts và Reviewer/Judge độc lập. Khoảng trống còn lại là lớp quản lý một **mục tiêu gồm nhiều việc**: phân rã, đồ thị phụ thuộc, khóa phạm vi, điều phối song song an toàn, trạng thái tổng hợp và lịch sử xuyên suốt.

## Thiết kế
`GoalRequest -> GoalDecomposer -> GoalPlan/DAG -> Ready Work -> Scope Lock + Lease -> Executor -> Evidence -> Reviewer -> Judge -> Completed`.

- `GoalDecomposer` là ranh giới tích hợp: Bộ điều phối AI có thể cung cấp việc phân rã sau này mà WO-044 không sửa logic chọn model/provider của WO-043.
...
- Idempotency ở cấp goal chống nhận trùng; workId là duy nhất toàn hệ thống.
