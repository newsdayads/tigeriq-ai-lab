export const CONTROL_TOWER_VIEW_MODEL_VERSION = 'tigeriq.company-control-tower.view.v2';
export const BUSINESS_STATE_V2_CONTRACT_SHA = '4bccf71d73c8d8cf100c65b935b3474f97f24459';
export const CHIEF_OF_STAFF_POLICY_V2_SHA = '0f673f92b703c8c67e8a89cb23a0c5f7307db3f2';

const asArray = value => Array.isArray(value) ? value : [];
const asObject = value => value && typeof value === 'object' && !Array.isArray(value) ? value : {};
const finite = value => value === null || value === undefined || value === '' ? null : (Number.isFinite(Number(value)) ? Number(value) : null);
const idOf = (row, ...keys) => keys.map(key => row?.[key]).find(Boolean) || null;
const isoTime = value => Number.isFinite(Date.parse(String(value || ''))) ? Date.parse(String(value)) : -Infinity;
...
function contractCandidate(snapshot) {
  const direct = asObject(snapshot?.businessStateV2);
  if (Object.keys(direct).length || snapshot?.businessStateV2) return direct;
...
function mapOwnerActions(exceptions) {
  return asArray(exceptions)
    .filter(row=>Boolean(row.requiredOwnerAction)&&['OPEN','AWAITING_OWNER'].includes(row.status))
    .map(row=>({...row,authorizationState:'BLOCKED_PENDING_OWNER_DECISION',ownerApprovalProof:{state:'NOT_ESTABLISHED',ref:null,rule:'decision_ref_is_not_owner_approval_ref'},policyBaseline:{sourcePr:154,exactSha:CHIEF_OF_STAFF_POLICY_V2_SHA}}));
}
...
function emptyWorkCoordination(reason = 'WORK_COORDINATION_SOURCE_PENDING') { return {available:false,reason,sourceSystem:'trello',readOnly:true,observedAt:null,provenance:null,cards:[]}; }
function mapWorkCoordination(snapshot,liveAuthoritative) {
  if (!liveAuthoritative) return emptyWorkCoordination('NON_AUTHORITATIVE_SOURCE');
  const raw = asObject(snapshot?.workCoordination), provenance = asObject(raw.provenance);
  if (raw.schemaVersion !== 'tigeriq.work-coordination.trello-readonly.v1' || raw.sourceSystem !== 'trello' || raw.readOnly !== true || provenance.source_system !== 'trello' || !provenance.source_ref || !Number.isFinite(Date.parse(String(provenance.observed_at || '')))) return emptyWorkCoordination();
  const cards = asArray(raw.cards).map(card => {
    const cardProvenance = asObject(card?.provenance);
    if (!card?.cardId || !card?.title || !card?.status || cardProvenance.source_system !== 'trello' || !cardProvenance.source_ref) return null;
    return {cardId:card.cardId,title:card.title,status:upper(card.status),dueAt:card.due_at || null,boardName:card.board_name || null,listName:card.list_name || null,members:asArray(card.members).filter(Boolean),provenance:cardProvenance};
  }).filter(Boolean);
  return {available:true,reason:null,sourceSystem:'trello',readOnly:true,observedAt:provenance.observed_at,provenance,cards};
}
