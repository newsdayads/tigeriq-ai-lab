...
// Cloud-first provider order. Models are intentionally resolved from environment/options
// instead of hard-coding fast-changing provider model IDs.
  ...
  async execute(request: ModelRequest): Promise<RoutedResult> {
    ...
    for (const target of routeCandidates(this.policy)) {
      ...
      try {
        ...
        if (!text.trim()) throw new ProviderRequestError(target.provider, 'invalid_response', 'empty model response');
        ...
        const immediateOpen = providerError && ['quota', 'outage', 'timeout', 'auth', 'configuration'].includes(providerError.kind);
...
function resolveModel(target: ModelTarget, sentinel: string, configured?: string): string {
  ...
  if (!model?.trim()) throw new ProviderRequestError(target.provider, 'configuration', `${target.provider} model not configured`);
  return model;
}
...
async function providerFetch(
  ...
  try {
    ...
    try {
      ...
      if (controller.signal.aborted) throw new ProviderRequestError(provider, 'timeout', `${provider} request timeout`);
...
export function createOpenAIAdapter(options: OpenAIAdapterOptions = {}): ProviderAdapter {
  ...
  return {
    ...
    async execute(target, request) {
      ...
      if (!apiKey) throw new ProviderRequestError('openai', 'configuration', 'openai api key not configured');
      const model = resolveModel(target, 'openai-default', options.model ?? process.env.TIGERIQ_OPENAI_MODEL);
      const response = await providerFetch('openai', fetchImpl, `${baseUrl}/chat/completions`, {
        method: 'POST',
        headers: { 'content-type': 'application/json', authorization: `Bearer ${apiKey}` },
        body: JSON.stringify({ model, messages: [{ role: 'user', content: request.prompt }], stream: false }),
      }, request, timeoutMs);
      const body = await response.json() as { choices?: Array<{ message?: { content?: string } }> };
      const text = body.choices?.[0]?.message?.content;
      if (!text?.trim()) throw new ProviderRequestError('openai', 'invalid_response', 'empty openai response');
      return text;
    },
  };
}
...
export function createAnthropicAdapter(options: AnthropicAdapterOptions = {}): ProviderAdapter {
  ...
  return {
    ...
    async execute(target, request) {
      ...
      if (!apiKey) throw new ProviderRequestError('anthropic', 'configuration', 'anthropic api key not configured');
      ...
      const response = await providerFetch('anthropic', fetchImpl, `${baseUrl}/messages`, {
        ...
        body: JSON.stringify({ model, max_tokens: maxTokens, messages: [{ role: 'user', content: request.prompt }] }),
      }, request, timeoutMs);
      const body = await response.json() as { content?: Array<{ type?: string; text?: string }> };
      const text = body.content?.filter((part) => part.type === 'text').map((part) => part.text ?? '').join('\n');
      if (!text?.trim()) throw new ProviderRequestError('anthropic', 'invalid_response', 'empty anthropic response');
      return text;
    },
  };
}
...
export function createGeminiAdapter(options: GeminiAdapterOptions = {}): ProviderAdapter {
  ...
  return {
    ...
    async execute(target, request) {
      ...
      if (!apiKey) throw new ProviderRequestError('gemini', 'configuration', 'gemini api key not configured');
      const model = resolveModel(target, 'gemini-default', options.model ?? process.env.TIGERIQ_GEMINI_MODEL);
      const response = await providerFetch('gemini', fetchImpl, `${baseUrl}/models/${encodeURIComponent(model)}:generateContent`, {
        method: 'POST',
      ...
      if (!text?.trim()) throw new ProviderRequestError('gemini', 'invalid_response', 'empty gemini response');
      return text;
    },
  };
}
...
export function createOllamaAdapter(options: OllamaAdapterOptions = {}): ProviderAdapter {
  ...
  return {
    ...
    async execute(target, request) {
      ...
      const body = await response.json() as { choices?: Array<{ message?: { content?: string } }> };
      const text = body.choices?.[0]?.message?.content;
      if (!text?.trim()) throw new ProviderRequestError('ollama', 'invalid_response', 'empty ollama response');
      return text;
    },
  };
}
