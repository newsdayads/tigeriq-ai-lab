...
export function latestLifecycleStage(comments = []) { const events = lifecycleEvents(comments); return events.length ? events[events.length - 1].stage : null; }
...
async function statusSnapshot(token = '') {
  ...
  const jobs = openIssues.filter((item) => !item.pull_request && typeof item.body === 'string' && (item.body.includes('TIGERIQ_JOB_V1') || item.body.includes('TIGERIQ_COMMAND_V1'))).slice(0, 20).map((item) => ({ number: item.number, title: item.title, state: item.state, updatedAt: item.updated_at, url: item.html_url, type: item.body.includes('TIGERIQ_COMMAND_V1') ? 'command' : 'work-order' }));
  ...
  return { ok: true, generatedAt: new Date().toISOString(), controlPlane: { vercel: 'online', github: 'online', repository: repoInfo.full_name, serverWriteConfigured: Boolean(GITHUB_TOKEN && COMMAND_SECRET), clientTokenSupported: true, chiefOfStaff: 'gpt', workOrderDedupe: true, workOrderStatusTracking: true, workOrderLifecycleEvidence: true, explicitDispatch: true, workBoard: true }, execution: { pc01, openclaw: 'unknown', ollama: 'unknown', canaryIssue: CANARY_ISSUE, canaryState: canary?.state || 'unknown' }, queue: { count: jobs.length, jobs } };
}
async function workBoard(token = '') {
  ...
  const markerIssues = (Array.isArray(issues) ? issues : []).filter((item) => !item.pull_request && typeof item.body === 'string' && (item.body.includes('TIGERIQ_JOB_V1') || item.body.includes('TIGERIQ_COMMAND_V1'))).slice(0, 20);
  ...
  return { ok: true, generatedAt: new Date(nowMs).toISOString(), policy: { staleMinutes: 30, issueLimit: 20, commentLimit: 100, mutation: false }, summary: { total: items.length, active: items.filter((item) => item.stage === 'queued' || item.stage === 'claimed').length, queued: count('queued'), claimed: count('claimed'), completed: count('completed'), failed: count('failed'), cancelled: count('cancelled'), stale: items.filter((item) => item.stale).length }, items };
}
async function githubIdentity(token) {
  if (!token) { const error = new Error('github_authorization_required'); error.status = 401; throw error; }
  const { owner, repo } = repoParts();
...
async function createWorkOrder(payload, token) {
  ...
  const governance = source === 'vercel-explicit-dispatch' ? 'Owner explicitly dispatched this instruction from TigerIQ AI Web Control. Execution still requires normal TigerIQ evidence/review/gate.' : 'Chief of Staff classified this as an explicit execution request. Execution still requires normal TigerIQ evidence/review/gate.';
  ...
  if (duplicate) return { ok: true, deduplicated: true, fingerprint, requestId: null, issue: { number: duplicate.number, url: duplicate.html_url, title: duplicate.title } };
  const id = randomUUID();
  const titleText = instruction.replace(/\s+/g, ' ').slice(0, 72);
  const body = ['TIGERIQ_JOB_V1', '', '## Instruction', instruction, '', '## Priority', priority, '', '## Source', source, '', '## Request ID', id, '', '## Fingerprint', fingerprint, '', '## Governance', governance].join('\n');
  const { owner, repo } = repoParts();
  const issue = await gh(`/repos/${owner}/${repo}/issues`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ title: `[${priority}] [TigerIQ AI] ${titleText}`, body }) }, token);
...
async function workOrderStatus(payload, token = '') {
  ...
  const { owner, repo } = repoParts();
  const [issue, comments] = await Promise.all([gh(`/repos/${owner}/${repo}/issues/${number}`, {}, token), gh(`/repos/${owner}/${repo}/issues/${number}/comments?per_page=100`, {}, token).catch(() => [])]);
  if (issue.pull_request || typeof issue.body !== 'string' || !(issue.body.includes('TIGERIQ_JOB_V1') || issue.body.includes('TIGERIQ_COMMAND_V1'))) { const error = new Error('invalid_work_order_issue'); error.status = 400; throw error; }
  const stage = issueStage(issue, comments);
  const evidence = issueEvidenceSummary(comments);
  return { ok: true, issue: { number: issue.number, title: issue.title, state: issue.state, stateReason: issue.state_reason || null, stage, url: issue.html_url, updatedAt: issue.updated_at, comments: Array.isArray(comments) ? comments.length : 0, evidence } };
}
async function createCanary(token) {
  ...
  const { owner, repo } = repoParts();
  const issue = await gh(`/repos/${owner}/${repo}/issues`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ title: `[P0] PC01 Web Control canary ${new Date().toISOString()}`, body }) }, token);
  return { ok: true, idempotencyKey: id, issue: { number: issue.number, url: issue.html_url } };
}
function formatStatusReply(snapshot) {
  const pc = snapshot.execution.pc01 === 'online' ? 'trực tuyến' : snapshot.execution.pc01 === 'working' ? 'đang làm việc' : snapshot.execution.pc01 === 'degraded' ? 'có lỗi' : 'ngắt kết nối';
...
export default async function handler(req, res) {
  try {
    if (req.method === 'GET') return json(res, 200, await statusSnapshot(clientGithubToken(req)));
    ...
    if (operation === 'chat') {
      ...
      if (command === '1') {
        if (!(authorizedByServerSecret(req) || isOwnerAuthorized(req))) return json(res, 401, { error: 'owner_authorization_required' });
        const plan = oneCommandWebControlPlan({ command: message, findings: [], backlog: [], authorization: { owner: true, offMain: true }, currentStage: 'queued' });
        return json(res, 202, { ok: true, mode: 'web-control', lane: 'web-control', command: '1', state: 'external-wait', plan, execution: { started: false, verified: false, reason: 'runtime_executor_unavailable' }, evidence: { source: 'web-control-loop', deterministic: true, offMain: true, executionVerified: false }, reply: `Web Control lane đã nhận lệnh ${message}; runtime executor chưa khả dụng nên dừng EXTERNAL_WAIT, không tạo Generic Work Order.` });
      }
      const decision = await decideWithChief({ message: payload.message, history: payload.history });
      if (decision.mode === 'status') { const snapshot = await statusSnapshot(optionalToken); return json(res, 200, { ok: true, mode: 'status', reply: formatStatusReply(snapshot), snapshot, modelUsed: decision.modelUsed, providerUsed: decision.providerUsed, usage: decision.usage }); }
      if (decision.mode === 'reply' || decision.mode === 'clarify') return json(res, 200, { ok: true, mode: decision.mode, reply: decision.reply, modelUsed: decision.modelUsed, providerUsed: decision.providerUsed, usage: decision.usage });
      ...
      const workReply = result.deduplicated ? `${decision.reply}\n\nCông việc này đang được theo dõi ở #${result.issue.number}; em không tạo bản trùng.` : `${decision.reply}\n\nĐã tạo công việc #${result.issue.number}. Em sẽ theo dõi execution → review → gate → evidence.`;
      return json(res, result.deduplicated ? 200 : 201, { ...result, mode: 'work-order', reply: workReply, modelUsed: decision.modelUsed, providerUsed: decision.providerUsed, usage: decision.usage });
    }
    if (operation === 'work-order') return json(res, (await createWorkOrder({ ...payload, source: 'vercel-explicit-dispatch' }, (await writeCredential(req)).token)).deduplicated ? 200 : 201, await createWorkOrder({ ...payload, source: 'vercel-explicit-dispatch' }, (await writeCredential(req)).token));
    ...
    if (name === 'payload_too_large') status = 413; else if (name.startsWith('invalid_')) status = 400; else if (name === 'github_authorization_required') status = 401; else if (name === 'github_401') status = 401; else if (name === 'github_403') status = 403;
