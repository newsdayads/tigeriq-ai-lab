...
const body={idempotencyKey:`nv02-groq-${run}`,title:'NV02 Groq Controller canary',objective:prompt,payload:{route:'groq',taskType:'ai',prompt},targetEmployeeId:'NV02',requiredPermissions:['cloud_ai:execute','evidence:write'],requiredCapabilities:['groq','evidence'],allowedWorkerKinds:['pc01'],expectedEvidence:['json'],scopeKeys:[`runtime/nv02-groq/${run}`],dependencies:[],maxAttempts:1,independentReview:false,judgeRequired:false,priority:'P0'};
...
const s=state.state;console.log(JSON.stringify({jobId,stage:s?.job?.stage,employeeId:s?.result?.employeeId,deviceId:s?.result?.deviceId,bindingId:s?.result?.bindingId,route:s?.result?.output?.route,provider:s?.result?.output?.provider,model:s?.result?.output?.model,content:s?.result?.output?.content,maxQueued,maxLeases,evidenceCount:(s?.evidence||[]).length,failure:s?.result?.failure}));
