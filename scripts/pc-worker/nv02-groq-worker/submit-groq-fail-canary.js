...
const prompt='';
const body={idempotencyKey:`nv02-groq-${run}`,title:'NV02 Groq failure guard canary',objective:'Reject empty payload prompt without provider work',payload:{route:'groq',taskType:'ai',prompt},targetEmployeeId:'NV02',requiredPermissions:['cloud_ai:execute','local_ai:execute','evidence:write'],requiredCapabilities:['groq','evidence'],allowedWorkerKinds:['pc01'],expectedEvidence:['json'],scopeKeys:[`runtime/nv02-groq/${run}`],dependencies:[],maxAttempts:1,independentReview:true,judgeRequired:true,priority:'P0'};
