...
Assert-True ([string]$config.providers.gemini_api.model -eq 'gemini-3.5-flash-lite') 'gemini_model_pinned'
...
Assert-True ($geminiInvokeText -match 'gemini-3\.5-flash-lite:generateContent') 'gemini_runtime_model_pinned'
...
Assert-True ([bool]$config.providers.groq.requireFreeTierProof) 'groq_free_tier_proof_required'
Assert-True (-not [bool]$config.providers.groq.allowPaidTier) 'groq_paid_tier_forbidden'
Assert-True ([string]$config.providers.groq.model -eq 'openai/gpt-oss-120b') 'groq_model_pinned'
Assert-True ([string]$config.providers.groq.serviceTier -eq 'on_demand') 'groq_on_demand_tier_pinned'
...
Assert-True ([string]$config.routing.fallbackOrder[1] -eq 'groq') 'groq_second_after_local'
...
Assert-True ($probeText -match 'function\s+Invoke-GroqFreeProbe') 'groq_free_probe_exists'
Assert-True ($probeText -match 'TIGERIQ_GROQ_FREE_TIER_VERIFIED') 'groq_free_tier_proof_guard_exists'
Assert-True ($probeText -match "service_tier='on_demand'") 'groq_service_tier_pinned_in_probe'
