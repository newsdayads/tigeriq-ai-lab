...
$script:LogPath=Join-Path $script:LogDir 'V14_2_2_INSTALL.log'
...
$tmp=Join-Path $env:TEMP ('TigerIQ_AW_14.2.2_'+[guid]::NewGuid().ToString('N'))
...
try{
  ...
  $libRaw=Get-Content -Raw -LiteralPath $lib
  $badReturnPattern='\breturn(?=[$@\[])'
  $badReturnCount=[regex]::Matches($libRaw,$badReturnPattern).Count
  if($badReturnCount-lt1){throw'EXPECTED_RETURN_TOKEN_DEFECT_NOT_FOUND_SOURCE_DRIFT'}
  $libFixed=[regex]::Replace($libRaw,$badReturnPattern,'return ').Replace('v14_2_1_','v14_2_2_')
  [IO.File]::WriteAllText($lib,$libFixed,(New-Object Text.UTF8Encoding($false)))
  if([regex]::IsMatch((Get-Content -Raw -LiteralPath $lib),$badReturnPattern)){throw'RETURN_TOKEN_FIXUP_FAILED'}
  $tokens=$null;$parseErrors=$null;[void][Management.Automation.Language.Parser]::ParseFile($lib,[ref]$tokens,[ref]$parseErrors)
  if(@($parseErrors).Count-gt0){throw('INSTALLER_LIBRARY_PARSE_FAILED '+(@($parseErrors|ForEach-Object{$_.Message})-join' | '))}
  . $lib
  ...
  Write-Log 'TIGERIQ AUTO WORKER 14.2.2 TEST CANDIDATE / PHYSICAL PENDING'
  ...
  $wrapper=$wrapper.Replace("const VERSION = '14.2.1';","const VERSION = '14.2.2';");$guard=$guard.Replace("const VERSION = '14.2.1';","const VERSION = '14.2.2';")
  if($wrapper-notmatch'TIQ142_WRAPPER'-or$wrapper-notmatch'14\.2\.2'){throw'WRAPPER_PAYLOAD_INVALID'}
  if($guard-notmatch'TIQ142_RUNTIME_GUARD'-or$guard-notmatch'14\.2\.2'){throw'RUNTIME_GUARD_PAYLOAD_INVALID'}
  ...
  if($env:TIQ_PREFLIGHT_ONLY-eq'1'){Write-Log ('WINDOWS_OWNER_CMD_PREFLIGHT=PASS return_token_repairs='+$badReturnCount);Write-Host 'PASS: V14.2.2 exact owner-CMD Windows preflight.';exit 0}
  ...
  if((Get-Content -Raw -LiteralPath $swPath)-notmatch'TIQ142_WRAPPER'){throw'SERVICE_WORKER_WRAPPER_VERIFY_FAILED'};if((Get-Content -Raw -LiteralPath (Join-Path $script:ExtensionPath 'tigeriq_aw_v142_runtime_guard.js'))-notmatch'14\.2\.2'){throw'RUNTIME_GUARD_VERIFY_FAILED'}
  ...
  Write-Host '';Write-Host 'PASS: TigerIQ Auto Worker 14.2.2 TEST CANDIDATE installed.';Write-Host 'PRE-ACTIVATION: NV02 only. NV04/NV05 NOT activated.';Write-Host ('Log: '+$script:LogPath);exit 0
}catch{
