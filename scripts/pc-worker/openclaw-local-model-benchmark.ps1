...
function Invoke-BenchRun([string]$model,[int]$ctx,[int]$runNo) {
  ...
  $errorText = $null
  try {
    $req = New-Object System.Net.Http.HttpRequestMessage([System.Net.Http.HttpMethod]::Post,"$OllamaBase/api/chat")
    $req.Content = $content
...
$report = [ordered]@{
  ...
  nextGate='Do not configure OpenClaw fallback until one profile has 3 consecutive PASS and OpenClaw-level local E2E/failover validation.'
}
