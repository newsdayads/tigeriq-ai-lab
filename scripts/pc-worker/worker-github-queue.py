...
BASE_MODEL = os.getenv('TIGERIQ_OLLAMA_MODEL', 'qwen2.5-coder:14b').strip()
EXECUTOR_MODEL = os.getenv('TIGERIQ_OLLAMA_EXECUTOR_MODEL', BASE_MODEL).strip()
REVIEWER_MODEL = os.getenv('TIGERIQ_OLLAMA_REVIEWER_MODEL', '').strip()
JUDGE_MODEL = os.getenv('TIGERIQ_OLLAMA_JUDGE_MODEL', '').strip()
REQUIRE_INDEPENDENT_AI = os.getenv('TIGERIQ_REQUIRE_INDEPENDENT_AI', '1').strip().lower() not in {'0', 'false', 'no'}
...
INDEPENDENCE_BLOCKED = 'TIGERIQ_PC01_INDEPENDENCE_BLOCKED'
...
def ollama_chat(model, messages, json_mode=False):
    if not model:
        raise RuntimeError('ollama model is not configured for this role')
...
def independent_model_gate():
    if not REQUIRE_INDEPENDENT_AI:
        return True, ''
    role_models = {
    ...
    missing = [role for role, model in role_models.items() if not model]
    if missing:
        return False, 'missing independent model configuration: ' + ','.join(missing)
    normalized = [model.lower() for model in role_models.values()]
    if len(set(normalized)) != 3:
        return False, 'executor, reviewer and judge must use three distinct model identities'
    return True, ''
...
def execute_with_tools(instruction):
    ...
    for step in range(1, MAX_STEPS + 1):
        raw = ollama_chat(EXECUTOR_MODEL, messages, json_mode=True)
        obj = parse_json(raw)
        if not isinstance(obj, dict):
            ...
            try:
                if action == 'read':
                    result = tool_read(obj.get('path', ''))
                elif action == 'list':
                    result = tool_list(obj.get('path', '.'))
                elif action == 'write':
                    result = tool_write(obj.get('path', ''), obj.get('content', ''))
                elif action == 'run':
                    result = run_command(obj.get('argv'), int(obj.get('timeout', 180)))
                elif action == 'finish':
        ...
        trace.append(
            f"step={step} action={obj.get('action') if isinstance(obj, dict) else 'invalid'} "
...
def execute_job(job):
    ...
    gate_ok, gate_reason = independent_model_gate()
    if not gate_ok:
        if INDEPENDENCE_BLOCKED not in comment_text:
            evidence = {
                'timestamp': now(),
                'worker': 'pc01',
                'provider': 'ollama',
                'policy': 'strict-independent-ai',
    ...
    reviewer_raw = ollama_chat(
        ...
        [
            {
                'role': 'system',
                'content': 'You are pc01-reviewer, independent from coder. Review only evidence in executor result. Reject unsupported claims. Return JSON only: {"pass":true|false,"reason":"..."}.',
            },
            {
                'role': 'user',
                'content': json.dumps({'instruction': instruction, 'executor_result': executor}, ensure_ascii=False),
            },
        ],
        json_mode=True,
    )
    ...
    judge_raw = ollama_chat(
        ...
        [
            {
                'role': 'system',
                'content': 'You are pc01-judge, independent from coder/reviewer. DONE requires instruction satisfied, concrete evidence, and reviewer PASS. Return JSON only: {"pass":true|false,"reason":"..."}.',
            },
            {
                'role': 'user',
                'content': json.dumps(
...
def main():
    ...
    print(
        f'{now()} TIGERIQ GITHUB QUEUE WORKER ONLINE repo={REPO} '
        f'executor_model={EXECUTOR_MODEL} reviewer_model={REVIEWER_MODEL or "UNCONFIGURED"} '
        f'judge_model={JUDGE_MODEL or "UNCONFIGURED"} mode=bounded-tools',
        flush=True,
    )
    while True:
        try:
