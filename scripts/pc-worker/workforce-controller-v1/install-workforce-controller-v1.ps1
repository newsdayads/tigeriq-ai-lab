param(
  ...
  [string]$ExpectedBranch = 'reset/m0-canonical-runtime',
  [switch]$StartNow
)
...
try {
  ...
  & $npm install --no-save --package-lock=false --ignore-scripts pg@8.16.3
  ...
  foreach ($migration in $Migrations) {
    $migrationPath = Join-Path $RepoPath $migration
    & (Join-Path $RepoPath 'scripts\install-work-state-postgres.ps1') -DatabaseUrl $DatabaseUrl -Migration $migrationPath
    if ($LASTEXITCODE -ne 0) { Fail 'POSTGRES_MIGRATION_FAILED' "Migration failed: $migration" }
  }
} finally {
  Pop-Location
}
