...
Assert-True ($installer -match 'DatabasePasswordFilePath' -and $installer -match 'LogonType S4U' -and $installer -notmatch "-UserId 'SYSTEM'") 'Named-user Controller runtime principal is not enforced.'
...
Assert-True ($postgresProvisioner -match 'workforce-controller-v1\.database-password\.secure' -and $postgresProvisioner -match 'DATABASE_CREDENTIAL_NOT_DURABLE') 'Durable encrypted PostgreSQL credential gate missing.'
...
Assert-True ($restart -match 'DatabasePasswordFile' -and $restart -match 'device_proof_replay_state') 'Restart verifier does not prove durable replay state with named-user DB auth.'
...
Assert-True ($allScripts -notmatch '(?i)PGPASSFILE|\.pgpass') 'PGPASSFILE/.pgpass authentication is forbidden.'
Assert-True ($allScripts -notmatch "New-ScheduledTaskPrincipal\s+-UserId\s+'SYSTEM'") 'SYSTEM scheduled-task authentication is forbidden.'
