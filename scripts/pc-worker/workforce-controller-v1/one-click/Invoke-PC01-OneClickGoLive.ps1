...
try {
  ...
  $dbProbe=(& $psql $effectiveDatabaseUrl -vON_ERROR_STOP=1 -Atc 'SELECT 1;' 2>$null).Trim(); if($LASTEXITCODE -ne 0 -or $dbProbe -ne '1'){Fail 'POSTGRES_CONNECTION_FAILED' 'Canonical TigerIQ PostgreSQL runtime connection failed.'}
  $migrationTable=(& $psql $effectiveDatabaseUrl -vON_ERROR_STOP=1 -Atc "SELECT to_regclass('public.tigeriq_schema_migrations') IS NOT NULL;" 2>$null).Trim(); if($migrationTable -eq 't'){ $existingVersions=@(& $psql $effectiveDatabaseUrl -vON_ERROR_STOP=1 -Atc 'SELECT version FROM tigeriq_schema_migrations ORDER BY version;' 2>$null | ForEach-Object {$_.Trim()} | Where-Object {$_}); if($existingVersions -contains $ForbiddenMigration){Fail 'MIGRATION_003_ALREADY_PRESENT' 'Forbidden business-state migration 003 is already present; refusing to proceed.'}; $unexpected=@($existingVersions|Where-Object{$_ -notin @('001_operational_state_v1','002_device_proof_replay_v1')}); if($unexpected.Count -gt 0){Fail 'UNREVIEWED_MIGRATION_PRESENT' 'Unreviewed PostgreSQL migration state exists; refusing runtime drift.'} }
