...
function Git-Text([string[]]$Args) {
  $result = (& $GitExecutable @Args 2>&1 | Out-String).Trim()
  if ($LASTEXITCODE -ne 0) { Fail 'GIT_COMMAND_FAILED' "git $($Args -join ' ') failed." }
  return $result
}
...
$remoteLine = Git-Text @('-C',$RepoPath,'ls-remote','--exit-code','origin',$ApprovedRemoteRef)
