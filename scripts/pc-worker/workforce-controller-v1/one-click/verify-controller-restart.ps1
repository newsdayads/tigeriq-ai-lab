...
$beforeReplay = (& $psql $DatabaseUrl -v ON_ERROR_STOP=1 -Atc "SELECT count(*) FROM device_proof_replay_state;" 2>$null).Trim()
if ($LASTEXITCODE -ne 0) { Fail 'REPLAY_STATE_QUERY_FAILED' 'Could not query durable replay state before restart.' }
$beforeMigrations = @(& $psql $DatabaseUrl -v ON_ERROR_STOP=1 -Atc "SELECT version FROM tigeriq_schema_migrations WHERE version IN ('001_operational_state_v1','002_device_proof_replay_v1') ORDER BY version;" 2>$null | ForEach-Object { $_.Trim() } | Where-Object { $_ })
if ($beforeMigrations.Count -ne 2) { Fail 'MIGRATION_STATE_INVALID' 'Required migrations are not present before restart.' }
...
do {
  ...
  $healthOutput = & $HealthScript -DatabaseUrl $DatabaseUrl 2>$null
  if ($LASTEXITCODE -eq 0) { $healthy = $true; break }
} while ((Get-Date) -lt $deadline)
if (-not $healthy) { Fail 'RESTART_HEALTH_FAILED' 'Controller did not return healthy after task restart.' }

$afterReplay = (& $psql $DatabaseUrl -v ON_ERROR_STOP=1 -Atc "SELECT count(*) FROM device_proof_replay_state;" 2>$null).Trim()
if ($LASTEXITCODE -ne 0) { Fail 'REPLAY_STATE_QUERY_FAILED' 'Could not query durable replay state after restart.' }
$afterMigrations = @(& $psql $DatabaseUrl -v ON_ERROR_STOP=1 -Atc "SELECT version FROM tigeriq_schema_migrations WHERE version IN ('001_operational_state_v1','002_device_proof_replay_v1') ORDER BY version;" 2>$null | ForEach-Object { $_.Trim() } | Where-Object { $_ })
if ($afterMigrations.Count -ne 2) { Fail 'MIGRATION_STATE_LOST' 'Required migration state changed across restart.' }
...
[ordered]@{
  ok = $true
  action = 'workforce.controller.v1.restart.verify'
  bind = "$ExpectedHost`:$ControllerPort"
  migrations = $afterMigrations
  replayStateRowsBefore = [int64]$beforeReplay
  replayStateRowsAfter = [int64]$afterReplay
  durableReplayStatePreserved = $true
  marker = 'PC01_CONTROLLER_RESTART_VERIFICATION_PASS'
