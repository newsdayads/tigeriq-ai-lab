...
if ($tailscaleOk -and $listenerOk) {
  try {
    ...
    $httpOk = [bool]$status.ok -and [bool]$status.postgres -and $status.migration -eq '001_operational_state_v1' -and $status.protocol -eq 'controller-v1'
...
[ordered]@{
  ...
  postgres = [ordered]@{ ok = $dbOk; migration = $dbMigration }
