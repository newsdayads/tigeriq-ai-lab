...
$httpOk = $false; $protocol = $null; $controllerMigration = $null; $statusPostgres = $false
if ($tailscaleOk -and $listenerOk) {
  try {
    ...
    $controllerMigration = [string]$status.migration
    $statusPostgres = [bool]$status.postgres
    $httpOk = [bool]$status.ok -and $statusPostgres -and $protocol -eq 'controller-v1' -and $controllerMigration -eq '001_operational_state_v1'
  } catch {}
}

$ok = $tailscaleOk -and $taskOk -and $listenerOk -and $firewallOk -and $dbOk -and -not $forbiddenApplied -and $httpOk
[ordered]@{
  ok=$ok; action='workforce.controller.v1.health'; expectedBind="$ExpectedHost`:$ControllerPort";
  ...
  postgres=[ordered]@{ok=$dbOk;migrations=$dbVersions;exactReviewedMigrations=$dbOk;replayTable=$replayTablePresent;forbidden003Applied=$forbiddenApplied;pgPassProtectedFilePresent=(Test-Path $PgPassFile)};
  http=[ordered]@{ok=$httpOk;protocol=$protocol;postgres=$statusPostgres;controllerMigrationField=$controllerMigration;migrationsVerifiedFromSamePostgres=$dbVersions}
