...
function Invoke-External([string]$Exe, [string[]]$ArgumentList, [int]$Timeout) {
  ...
  try {
    ...
    return [ordered]@{ exitCode=[int]$process.ExitCode; timeout=$false; output=(Sanitize $outText); error=(Sanitize $errText) }
  } finally {
    $process.Dispose()
  }
}
...
function Set-LiveResult([System.Collections.IDictionary]$Result, [System.Collections.IDictionary]$LiveResult, [string]$ExpectedMarker) {
  ...
  $Result.status = if ($LiveResult.timeout) {
    ...
    if ($combined -match '^BILLING_ROUTE_BLOCKED|CLAUDE_SUBSCRIPTION_AUTH_UNPROVEN|CLAUDE_USAGE_CREDITS_STATUS_UNPROVEN|OPENROUTER_(NONFREE_MODEL|COST_UNPROVEN|COST_NONZERO)') { 'BILLING_ROUTE_BLOCKED' } else { 'AUTH_OR_INVOCATION_ERROR' }
  } else {
    'UNEXPECTED_RESPONSE'
  }
...
function Run-SelfTest {
  ...
  $cmd = Resolve-Tool @('cmd.exe','cmd')
  if ($cmd) {
    $exitZero = Invoke-External -Exe $cmd -ArgumentList @('/d','/s','/c','"exit 0"') -Timeout 5
