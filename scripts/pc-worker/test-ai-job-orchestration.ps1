...
try {
  ...
  $result = Invoke-AiJobOrchestration -StatePath $statePath -EvidencePath $evidencePath -WorkOrderId 'JOB-001' -CandidatesByRole $candidates -EligibleBackendIdentities $eligible -ProviderInvoker $invoker -MaxAttempts 3 -LeaseSeconds 30
  ...
  $restart = Invoke-AiJobOrchestration -StatePath $statePath -EvidencePath $evidencePath -WorkOrderId 'JOB-001' -CandidatesByRole $candidates -EligibleBackendIdentities $eligible -ProviderInvoker $noCallInvoker -MaxAttempts 3 -LeaseSeconds 30
  ...
  $terminal = Invoke-AiJobOrchestration -StatePath $terminalState -EvidencePath $terminalEvidence -WorkOrderId 'JOB-TERM' -CandidatesByRole $candidates -EligibleBackendIdentities $eligible -ProviderInvoker $terminalInvoker -MaxAttempts 3 -LeaseSeconds 30
  ...
  $paidCandidates = @{ executor=@('paid:model'); reviewer=@('paid:model'); judge=@('paid:model') }
  $eligCalls = 0
  $eligInvoker = { param($role,$backend,$attempt) $script:eligCalls++; return [pscustomobject]@{ok=$true;failureClass='';output='x'} }
  $elig = Invoke-AiJobOrchestration -StatePath $eligState -EvidencePath $eligEvidence -WorkOrderId 'JOB-ELIG' -CandidatesByRole $paidCandidates -EligibleBackendIdentities $eligible -ProviderInvoker $eligInvoker -MaxAttempts 3 -LeaseSeconds 30
