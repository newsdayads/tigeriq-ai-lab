...
try {
  ...
  $a1 = Invoke-SchedulerAction -State $state -Action Acquire -WorkOrderId 'WO-TEST-1' -Role executor -Candidates @('openrouter:free','ollama:qwen') -ExcludedBackendIdentities @() -MaxAttempts 3 -LeaseSeconds 30 -NowUtc $t0
  ...
  $held = Invoke-SchedulerAction -State $state -Action Acquire -WorkOrderId 'WO-TEST-1' -Role executor -Candidates @('ollama:qwen') -MaxAttempts 3 -LeaseSeconds 30 -NowUtc $t0.AddSeconds(5)
  ...
  $a2 = Invoke-SchedulerAction -State $state -Action Acquire -WorkOrderId 'WO-TEST-1' -Role executor -Candidates @('ollama:qwen') -MaxAttempts 3 -LeaseSeconds 30 -NowUtc $t0.AddSeconds(8)
  ...
  $a3 = Invoke-SchedulerAction -State $state -Action Acquire -WorkOrderId 'WO-TEST-1' -Role executor -Candidates @('gemini:account') -MaxAttempts 3 -LeaseSeconds 30 -NowUtc $t0.AddSeconds(10)
  ...
  $blocked = Invoke-SchedulerAction -State $state -Action Acquire -WorkOrderId 'WO-TEST-1' -Role executor -Candidates @('ollama:qwen') -MaxAttempts 3 -LeaseSeconds 30 -NowUtc $t0.AddSeconds(12)
  Assert-True ($blocked.status -eq 'BLOCKED_TERMINAL') 'exhausted_stays_blocked'

  $review = Invoke-SchedulerAction -State $state -Action Acquire -WorkOrderId 'WO-TEST-2' -Role reviewer -Candidates @('openrouter:free','ollama:qwen') -ExcludedBackendIdentities @('openrouter:free') -MaxAttempts 3 -LeaseSeconds 30 -NowUtc $t0
  ...
  $deduped = Invoke-SchedulerAction -State $state -Action Acquire -WorkOrderId 'WO-TEST-2' -Role reviewer -Candidates @('gemini:account') -MaxAttempts 3 -LeaseSeconds 30 -NowUtc $t0.AddSeconds(2)
  Assert-True ($deduped.status -eq 'DEDUPED_COMPLETED') 'completed_work_deduped'

  $expired1 = Invoke-SchedulerAction -State $state -Action Acquire -WorkOrderId 'WO-TEST-3' -Role judge -Candidates @('ollama:qwen') -MaxAttempts 3 -LeaseSeconds 5 -NowUtc $t0
  Assert-True ($expired1.status -eq 'LEASE_ACQUIRED') 'expiring_lease_acquired'
  $expired2 = Invoke-SchedulerAction -State $state -Action Acquire -WorkOrderId 'WO-TEST-3' -Role judge -Candidates @('gemini:account') -MaxAttempts 3 -LeaseSeconds 5 -NowUtc $t0.AddSeconds(6)
  ...
  $terminal = Invoke-SchedulerAction -State $state -Action Acquire -WorkOrderId 'WO-TEST-4' -Role executor -Candidates @('openrouter:free') -MaxAttempts 3 -LeaseSeconds 30 -NowUtc $t0
  ...
  $fileAcquire = Invoke-ProviderSchedulerFile -StatePath $statePath -Action Acquire -WorkOrderId 'WO-FILE-1' -Role executor -Candidates @('ollama:qwen') -MaxAttempts 3 -LeaseSeconds 30 -LockWaitMs 1000
  Assert-True ($fileAcquire.status -eq 'LEASE_ACQUIRED') 'file_backed_acquire'
  Assert-True (Test-Path $statePath) 'file_state_written'
  $fileHeld = Invoke-ProviderSchedulerFile -StatePath $statePath -Action Acquire -WorkOrderId 'WO-FILE-1' -Role executor -Candidates @('gemini:account') -MaxAttempts 3 -LeaseSeconds 30 -LockWaitMs 1000
