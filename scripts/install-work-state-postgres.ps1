...
if (-not $psql) {
  foreach ($candidate in @('C:\Program Files\PostgreSQL\17\bin\psql.exe','C:\Program Files\PostgreSQL\16\bin\psql.exe')) {
    if (Test-Path $candidate) { $psql = Get-Item $candidate; break }
  }
}
if (-not $psql) { throw "psql.exe is not available. Install PostgreSQL locally on PC01 first." }
...
foreach ($path in @($Migration,$ReplayMigration)) {
  & $psql.Source --dbname=$DatabaseUrl -v ON_ERROR_STOP=1 -f $path
  if ($LASTEXITCODE -ne 0) { throw "PostgreSQL migration failed with exit code $LASTEXITCODE for $path" }
}
$versionCount = (& $psql.Source --dbname=$DatabaseUrl -v ON_ERROR_STOP=1 -Atc "SELECT count(*) FROM tigeriq_schema_migrations WHERE version IN ('001_operational_state_v1','002_device_proof_replay_v1');").Trim()
