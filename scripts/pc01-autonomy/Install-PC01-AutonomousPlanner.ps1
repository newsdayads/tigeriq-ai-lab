...
foreach($p in @($RepoPath,$Template,$TokenFile)){if(-not(Test-Path $p)){Fail 'REQUIRED_PATH_MISSING' $p}}
...
Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $startup -Settings $settings -Principal $taskPrincipal|Out-Null;Start-ScheduledTask -TaskName $TaskName
