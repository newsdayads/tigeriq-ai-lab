param([string]$RepoPath='F:\TigerIQ\Workspace\tigeriq-ai-lab',[string]$ExpectedBranch='wo060/mission-decomposition-v1')
...
& $powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $plannerInstaller -RepoPath $RepoPath -ExpectedBranch $ExpectedBranch;if($LASTEXITCODE -ne 0){Fail 'PLANNER_INSTALL_FAILED' 'Autonomous Planner install failed'}
...
foreach($pair in @(@($MissionTask,$MissionRunner),@($SupervisorTask,$SupervisorRunner))){$name=$pair[0];$runner=$pair[1];$existing=Get-ScheduledTask -TaskName $name -ErrorAction SilentlyContinue;if($existing){Stop-ScheduledTask -TaskName $name -ErrorAction SilentlyContinue;Unregister-ScheduledTask -TaskName $name -Confirm:$false};$action=New-ScheduledTaskAction -Execute $powershell -Argument "-NoProfile -NonInteractive -ExecutionPolicy Bypass -File `"$runner`"";Register-ScheduledTask -TaskName $name -Action $action -Trigger $startup -Settings $settings -Principal $taskPrincipal|Out-Null;Start-ScheduledTask -TaskName $name}
$deadline=(Get-Date).AddSeconds(120);do{$missionReady=$false;$supervisorReady=$false;if(Test-Path $MissionState){try{$ms=Get-Content $MissionState -Raw|ConvertFrom-Json;$missionReady=[bool]$ms.lastCycleAt}catch{}};if(Test-Path $SupervisorState){try{$ss=Get-Content $SupervisorState -Raw|ConvertFrom-Json;$supervisorReady=[bool]$ss.overallOk}catch{}};if($missionReady -and $supervisorReady){break};Start-Sleep -Seconds 2}while((Get-Date)-lt $deadline)
if(-not $missionReady){Fail 'MISSION_START_TIMEOUT' 'Mission Orchestrator did not produce runtime state'};if(-not $supervisorReady){Fail 'SUPERVISOR_START_TIMEOUT' 'Autonomy Supervisor did not become healthy'}
[ordered]@{ok=$true;status='PC01_AUTONOMY_FINAL_INSTALLED';branch=$branch;planner='TigerIQ Autonomous Planner';missionOrchestrator=$MissionTask;supervisor=$SupervisorTask;missionInbox=$Inbox;missionState=$MissionState;supervisorState=$SupervisorState;mainProductionTouched=$false;secretPrinted=$false}|ConvertTo-Json -Compress
