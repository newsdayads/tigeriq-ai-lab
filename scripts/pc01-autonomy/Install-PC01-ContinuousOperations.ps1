...
$installStarted=(Get-Date).ToUniversalTime();$deadline=(Get-Date).AddSeconds(90);$ready=$false;do{$taskState=Get-ScheduledTask -TaskName $Task -ErrorAction SilentlyContinue;if(Test-Path $State){try{$s=Get-Content $State -Raw|ConvertFrom-Json;if($s.lastCycleAt){$t=[datetime]::Parse([string]$s.lastCycleAt).ToUniversalTime();$ready=($t -ge $installStarted -and $taskState.State -eq 'Running')}}catch{}};if($ready){break};Start-Sleep -Seconds 2}while((Get-Date)-lt $deadline)
if(-not $ready){$tail='';if(Test-Path $Log){$tail=((Get-Content $Log -Tail 12)-join ' | ')};Fail 'CONTINUOUS_START_TIMEOUT' "Continuous Operations did not produce fresh state. task=$($taskState.State); log=$tail"}
