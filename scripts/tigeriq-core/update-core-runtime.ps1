  ...
  try{
    ...
    try{
      ...
      if($impact.updater -or $impact.openclaw){
        ...
        if(-not $openclawCanary -or [string]$openclawCanary.result -ne 'PASS'){Invoke-LiveStatusBridgeReconcile;if($?){$liveStatusBridgeReconcile=Invoke-LiveStatusBridgeReconcile}else{exit 75}
