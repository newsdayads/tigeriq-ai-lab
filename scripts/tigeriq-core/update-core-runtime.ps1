param([int]$IntervalSeconds=120,[string]$ApprovedReleaseSha='')
...
function Gates-Pass([string]$sha){
  if($sha -notmatch '^[0-9a-f]{40}function Get-NodePidByMatch([string]$match){
...
function Sync-WebRuntime(){
  ...
  $files=@(
    ...
    @{src='apps\tigeriq-core\execution-policy.mjs';dst='execution-policy.mjs'},
...
function Ensure-ServiceHealth([string]$key,[string]$url){
  if($key -eq 'coding'){return @{service='coding';healthy=$true;enabled=$false;action='disabled'}}
  $h=HealthInfo $url
...
function Get-Impact([string[]]$paths){
  ...
  $coding=$false # Legacy lane is never restarted, including rollback paths.
...
while($true){
  ...
  try{
    ...
    if($ApprovedReleaseSha -ne $remote){Save-State @{result='AWAITING_RELEASE_APPROVAL';candidateSha=$remote;watchdog=$watchdog};continue}
  ...
  $runs=gh api "repos/newsdayads/tigeriq-ai-lab/actions/runs?head_sha=$sha&per_page=100"|ConvertFrom-Json
  if($LASTEXITCODE -ne 0){return $false}
  $need=@('CI','WO-014 Queue Hygiene','WO-012/013 Vercel Online Verify')
  foreach($n in $need){
    $latest=@($runs.workflow_runs|Where-Object{$_.name -eq $n -and $_.head_sha -eq $sha}|Sort-Object -Property @{Expression='run_number';Descending=$true},@{Expression='run_attempt';Descending=$true}|Select-Object -First 1)
    if($latest.Count -ne 1 -or $latest[0].status -ne 'completed' -or $latest[0].conclusion -ne 'success'){return $false}
  }
  return $true
}
function Resolve-GateSha([string]$remote){
  # A successful historical PR head is not evidence for the deployed commit.
