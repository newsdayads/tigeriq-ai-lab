...
function Ensure-ServiceHealth([string]$key,[string]$url){
  if($key -eq 'coding'){return @{service=$key;healthy=$true;action='disabled';reason='LEGACY_AUTONOMY_QUARANTINED_STEP1'}}
  $h=HealthInfo $url
...
function Get-Impact([string[]]$paths){
  $web=[bool](@($paths|Where-Object{$_ -eq 'apps/tigeriq-core/execution-policy.mjs' -or $_ -match '^apps/tigeriq-core/web-control(?:\.|-)' -or $_ -match '^scripts/tigeriq-core/(?:run|install)-web-control'}).Count)
  $coding=$false
