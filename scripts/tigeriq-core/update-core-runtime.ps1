...
$appChromeInstallRequest='D:\TigerIQ\State\appchrome-install-request.json'
$appChromeInstallResult='D:\TigerIQ\State\appchrome-install-result.json'
$appChromeInstallStageRoot='D:\TigerIQ\State\AppChromeInstall'
...
function Save-AppChromeInstallResult([string]$result,[string]$reason,[string]$exactHead,[long]$artifactId,[int]$issueNumber,$details=$null){
  $d=[ordered]@{
    schema='TIGERIQ_APP_CHROME_INSTALL_RESULT_V1'
    result=$result
    reason=$reason
    exactHead=$exactHead
  ...
  $tmp=$appChromeInstallResult+'.tmp'
  [IO.File]::WriteAllText($tmp,($d|ConvertTo-Json -Depth 10),(New-Object Text.UTF8Encoding($false)))
  Move-Item -Force $tmp $appChromeInstallResult
}
function Get-AppChromeInstallResult(){
  try{if(Test-Path -LiteralPath $appChromeInstallResult){return (Get-Content -LiteralPath $appChromeInstallResult -Raw|ConvertFrom-Json)}}catch{}
  return $null
}
function Get-AppChromeInstallRequest(){
  if(-not(Test-Path -LiteralPath $appChromeInstallRequest)){return $null}
  $req=Get-Content -LiteralPath $appChromeInstallRequest -Raw|ConvertFrom-Json -ErrorAction Stop
  if([string]$req.schema -ne 'TIGERIQ_APP_CHROME_INSTALL_REQUEST_V1'){throw 'APPCHROME_REQUEST_SCHEMA_INVALID'}
  $head=[string]$req.exactHead
  $artifactName=[string]$req.artifactName
  $artifactId=[long]$req.artifactId
...
while($true){
  ...
  try{
    ...
    $appChromeInstall=Invoke-AppChromeInstallRequest
...
){throw 'APPCHROME_REQUEST_HEAD_INVALID'}
  if($artifactId -le 0){throw 'APPCHROME_REQUEST_ARTIFACT_ID_INVALID'}
  if($issueNumber -le 0){throw 'APPCHROME_REQUEST_ISSUE_INVALID'}
  if($artifactName -notmatch '^TigerIQ-Chrome-Controller-V1-\d+
function Invoke-LiveStatusBridgeReconcile(){
  try {
    if(-not (Task-Exists $liveStatusBridgeTask)) {
      return [ordered]@{status='BLOCKED';reason='TASK_ABSENT';action='NONE'}
...
){throw 'APPCHROME_REQUEST_ARTIFACT_NAME_INVALID'}
  return [pscustomobject]@{schema=[string]$req.schema;exactHead=$head;artifactId=$artifactId;artifactName=$artifactName;issueNumber=$issueNumber}
}
function Assert-AppChromeOwnerDeployAuthorization($req){
  $raw=(& gh issue view ([int]$req.issueNumber) --repo newsdayads/tigeriq-ai-lab --json state,body 2>$null|Out-String).Trim()
  if($LASTEXITCODE -ne 0 -or -not $raw){throw 'APPCHROME_OWNER_AUTH_ISSUE_UNAVAILABLE'}
  $issue=$raw|ConvertFrom-Json -ErrorAction Stop
  if([string]$issue.state -ne 'OPEN'){throw 'APPCHROME_OWNER_AUTH_ISSUE_NOT_OPEN'}
  $body=[string]$issue.body
  foreach($required in @(
    '(?m)^OWNER_DIRECT=true\s*
function Invoke-LiveStatusBridgeReconcile(){
  try {
    if(-not (Task-Exists $liveStatusBridgeTask)) {
      return [ordered]@{status='BLOCKED';reason='TASK_ABSENT';action='NONE'}
...
,
    '(?m)^APP_CHROME_DEPLOY_AUTHORIZED=true\s*
function Invoke-LiveStatusBridgeReconcile(){
  try {
    if(-not (Task-Exists $liveStatusBridgeTask)) {
      return [ordered]@{status='BLOCKED';reason='TASK_ABSENT';action='NONE'}
...
,
    '(?m)^MUTATION_OWNER=VY_OWNER_AUTHORIZED\s*
function Invoke-LiveStatusBridgeReconcile(){
  try {
    if(-not (Task-Exists $liveStatusBridgeTask)) {
      return [ordered]@{status='BLOCKED';reason='TASK_ABSENT';action='NONE'}
...
,
    ('(?m)^TARGET_HEAD='+[regex]::Escape([string]$req.exactHead)+'\s*
function Invoke-LiveStatusBridgeReconcile(){
  try {
    if(-not (Task-Exists $liveStatusBridgeTask)) {
      return [ordered]@{status='BLOCKED';reason='TASK_ABSENT';action='NONE'}
...
while($true){
  ...
  Start-Sleep -Seconds $IntervalSeconds
    ...
    ('(?m)^PACKAGE_ARTIFACT_ID='+[regex]::Escape([string]$req.artifactId)+'\s*
function Invoke-LiveStatusBridgeReconcile(){
  try {
    if(-not (Task-Exists $liveStatusBridgeTask)) {
      return [ordered]@{status='BLOCKED';reason='TASK_ABSENT';action='NONE'}
...
while($true){
  ...
  Start-Sleep -Seconds $IntervalSeconds
    ...
    if($body -notmatch $required){throw 'APPCHROME_OWNER_AUTH_CONTRACT_MISMATCH'}
  }
  return $true
}
function Get-VerifiedAppChromeArtifact($req){
  $artifactRaw=(& gh api ("repos/newsdayads/tigeriq-ai-lab/actions/artifacts/"+[string]$req.artifactId) 2>$null|Out-String).Trim()
  if($LASTEXITCODE -ne 0 -or -not $artifactRaw){throw 'APPCHROME_ARTIFACT_METADATA_UNAVAILABLE'}
  $artifact=$artifactRaw|ConvertFrom-Json -ErrorAction Stop
  ...
  $runId=[long]$artifact.workflow_run.id
  if($runId -le 0){throw 'APPCHROME_ARTIFACT_RUN_ID_MISSING'}
  $runRaw=(& gh api ("repos/newsdayads/tigeriq-ai-lab/actions/runs/"+[string]$runId) 2>$null|Out-String).Trim()
  if($LASTEXITCODE -ne 0 -or -not $runRaw){throw 'APPCHROME_ARTIFACT_RUN_UNAVAILABLE'}
  $run=$runRaw|ConvertFrom-Json -ErrorAction Stop
  if([string]$run.name -ne 'Chrome Controller Package'){throw 'APPCHROME_ARTIFACT_WORKFLOW_INVALID'}
  if([string]$run.conclusion -ne 'success'){throw 'APPCHROME_ARTIFACT_WORKFLOW_NOT_SUCCESS'}
  if([string]$run.head_sha -ne [string]$req.exactHead){throw 'APPCHROME_ARTIFACT_HEAD_MISMATCH'}
  if(-not(Gates-Pass ([string]$req.exactHead))){throw 'APPCHROME_EXACT_HEAD_GATES_NOT_PASS'}
  return [pscustomobject]@{runId=$runId;artifactName=[string]$artifact.name;head=[string]$run.head_sha}
}
function Wait-AppChromeSafeBoundary([int]$timeoutSec=600){
  $deadline=(Get-Date).AddSeconds($timeoutSec)
  while((Get-Date)-lt$deadline){
    try{
      $s=Invoke-RestMethod -Uri ($appChromeController+'/api/state') -TimeoutSec 5
      $busy=@($s.workers|Where-Object{[string]$_.status -eq 'WORKING' -or [bool]$_.lastHeartbeat.uiBusy -or [bool]$_.lastHeartbeat.stopVisible})
      if($busy.Count -gt 0){Start-Sleep -Seconds 3;continue}
      Invoke-RestMethod -Method Post -Uri ($appChromeController+'/api/pause') -TimeoutSec 5|Out-Null
      Start-Sleep -Milliseconds 750
      $locked=Invoke-RestMethod -Uri ($appChromeController+'/api/state') -TimeoutSec 5
      $race=@($locked.workers|Where-Object{[string]$_.status -eq 'WORKING' -or [bool]$_.lastHeartbeat.uiBusy -or [bool]$_.lastHeartbeat.stopVisible})
      if($race.Count -eq 0){return $locked}
      Invoke-RestMethod -Method Post -Uri ($appChromeController+'/api/resume') -TimeoutSec 5|Out-Null
    }catch{}
    Start-Sleep -Seconds 3
  }
  throw 'APPCHROME_SAFE_BOUNDARY_TIMEOUT'
}
function Wait-AppChromeExactHead([string]$exactHead,[int]$timeoutSec=150){
  $deadline=(Get-Date).AddSeconds($timeoutSec)
  while((Get-Date)-lt$deadline){
    try{
      $state=Invoke-RestMethod -Uri ($appChromeController+'/api/state') -TimeoutSec 5
      $bridge=Invoke-RestMethod -Uri 'http://127.0.0.1:8799/health' -TimeoutSec 5
      if([string]$state.runtimeProvenance.approvedHead -eq $exactHead -and [string]$bridge.approvedHead -eq $exactHead -and [bool]$bridge.provenanceVerified -and -not[bool]$state.externalWorkAutopilotEnabled -and -not[bool]$state.selfRun.enabled -and -not[bool]$state.githubSelfRun.enabled){
        return [pscustomobject]@{state=$state;bridge=$bridge}
      }
    }catch{}
    Start-Sleep -Seconds 2
  }
...
function Invoke-AppChromeInstallRequest(){
  $req=$null
  try{$req=Get-AppChromeInstallRequest}catch{
    Save-AppChromeInstallResult 'BLOCKED' $_.Exception.Message '' 0 0
    return @{action='blocked';reason=$_.Exception.Message}
  }
  if($null -eq $req){return @{action='none';reason='request_absent'}}
  $prior=Get-AppChromeInstallResult
  if($prior -and [string]$prior.result -eq 'PASS' -and [string]$prior.exactHead -eq [string]$req.exactHead -and [long]$prior.artifactId -eq [long]$req.artifactId){
    return @{action='none';reason='already_installed';exactHead=[string]$req.exactHead;artifactId=[long]$req.artifactId}
  }
  $runtime='D:\TigerIQ\Apps\ChromeController\Runtime'
  $activePath=Join-Path $runtime 'active-deploy.json'
  $launcherPath=Join-Path $runtime 'Start-Unified-AppChrome.ps1'
  $launcherLegacy=Join-Path $runtime 'Start-Unified-AppChrome-1372.ps1'
  $taskName='TigerIQ APP Chrome Unified'
  $stage=Join-Path $appChromeInstallStageRoot (([string]$req.exactHead).Substring(0,7)+'-'+[string]$req.artifactId)
  $rollback=Join-Path $stage 'rollback'
  $paused=$false
  try{
    Assert-AppChromeOwnerDeployAuthorization $req|Out-Null
    $verified=Get-VerifiedAppChromeArtifact $req
    if(Test-Path -LiteralPath $stage){Remove-Item -LiteralPath $stage -Recurse -Force}
    New-Item -ItemType Directory -Path $stage -Force|Out-Null
    & gh run download ([long]$verified.runId) --repo newsdayads/tigeriq-ai-lab --name ([string]$req.artifactName) --dir $stage 2>$null
    ...
    if(-not(Test-Path -LiteralPath (Join-Path $artifactRoot 'VERSION.txt'))){
      $nested=Join-Path $stage 'TigerIQ-Chrome-Controller-V1'
      if(Test-Path -LiteralPath (Join-Path $nested 'VERSION.txt')){$artifactRoot=$nested}
    }
    $version=Join-Path $artifactRoot 'VERSION.txt'
    if(-not(Test-Path -LiteralPath $version)){throw 'APPCHROME_DOWNLOADED_VERSION_MISSING'}
    if((Get-Content -LiteralPath $version -Raw).Trim() -ne [string]$req.exactHead){throw 'APPCHROME_DOWNLOADED_HEAD_MISMATCH'}
    $installer=Join-Path $artifactRoot 'apps\chrome-controller\runtime\Install-ApprovedArtifact.ps1'
    if(-not(Test-Path -LiteralPath $installer)){throw 'APPCHROME_CANONICAL_INSTALLER_MISSING'}
    Wait-AppChromeSafeBoundary|Out-Null
    $paused=$true
    New-Item -ItemType Directory -Path $rollback -Force|Out-Null
    if(Test-Path -LiteralPath $activePath){Copy-Item -LiteralPath $activePath -Destination (Join-Path $rollback 'active-deploy.json') -Force}
    if(Test-Path -LiteralPath $launcherPath){Copy-Item -LiteralPath $launcherPath -Destination (Join-Path $rollback 'Start-Unified-AppChrome.ps1') -Force}
    if(Test-Path -LiteralPath $launcherLegacy){Copy-Item -LiteralPath $launcherLegacy -Destination (Join-Path $rollback 'Start-Unified-AppChrome-1372.ps1') -Force}
    ...
    $live=Wait-AppChromeExactHead ([string]$req.exactHead)
    try{Invoke-RestMethod -Method Post -Uri ($appChromeController+'/api/resume') -TimeoutSec 5|Out-Null}catch{}
    $details=[ordered]@{runId=[long]$verified.runId;deploy=[string]$live.state.runtimeProvenance.deployRoot;bridgeSha256=[string]$live.bridge.sourceSha256;provenanceVerified=[bool]$live.bridge.provenanceVerified}
    Save-AppChromeInstallResult 'PASS' 'APPCHROME_EXACT_HEAD_LIVE' ([string]$req.exactHead) ([long]$req.artifactId) ([int]$req.issueNumber) $details
    $body=@(
      'APP_CHROME_ZERO_TOUCH_INSTALL=PASS',
      ('TARGET_HEAD='+[string]$req.exactHead),
      ('PACKAGE_ARTIFACT_ID='+[string]$req.artifactId),
      ('PACKAGE_RUN='+[string]$verified.runId),
      ('RUNTIME_DEPLOY='+[string]$live.state.runtimeProvenance.deployRoot),
      ('PROVENANCE_VERIFIED='+[string][bool]$live.bridge.provenanceVerified),
      'SELF_RUN=false',
      'GITHUB_SELF_RUN=false',
      'EXTERNAL_WORK_AUTOPILOT=false'
    ) -join [Environment]::NewLine
    & gh issue comment ([int]$req.issueNumber) --repo newsdayads/tigeriq-ai-lab --body $body 2>$null|Out-Null
    return @{action='installed';result='PASS';exactHead=[string]$req.exactHead;artifactId=[long]$req.artifactId;runId=[long]$verified.runId}
  }catch{
    $reason=$_.Exception.Message
    try{
      $rollbackActive=Join-Path $rollback 'active-deploy.json'
      if(Test-Path -LiteralPath $rollbackActive){Copy-Item -LiteralPath $rollbackActive -Destination $activePath -Force}
      $rollbackLauncher=Join-Path $rollback 'Start-Unified-AppChrome.ps1'
      if(Test-Path -LiteralPath $rollbackLauncher){Copy-Item -LiteralPath $rollbackLauncher -Destination $launcherPath -Force}
      $rollbackLegacy=Join-Path $rollback 'Start-Unified-AppChrome-1372.ps1'
      if(Test-Path -LiteralPath $rollbackLegacy){Copy-Item -LiteralPath $rollbackLegacy -Destination $launcherLegacy -Force}
      if(Task-Exists $taskName){Stop-ScheduledTask -TaskName $taskName -ErrorAction SilentlyContinue;Start-ScheduledTask -TaskName $taskName}
      try{Invoke-RestMethod -Method Post -Uri ($appChromeController+'/api/resume') -TimeoutSec 5|Out-Null}catch{}
    }catch{}
    Save-AppChromeInstallResult 'BLOCKED' $reason ([string]$req.exactHead) ([long]$req.artifactId) ([int]$req.issueNumber)
    return @{action='blocked';result='BLOCKED';reason=$reason;exactHead=[string]$req.exactHead;artifactId=[long]$req.artifactId}
  }
}
