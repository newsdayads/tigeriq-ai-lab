...
while($true){
  ...
  try{
    ...
    $healthy=$false;$deadline=(Get-Date).AddSeconds(60);while((Get-Date)-lt$deadline){if(Health){$healthy=$true;break};Start-Sleep -Seconds 2}
    if(-not $healthy){git -C $repo reset --hard $local|Out-Null;Stop-ScheduledTask -TaskName $task -ErrorAction SilentlyContinue;Start-Sleep -Seconds 2;Start-ScheduledTask -TaskName $task;throw 'HEALTH_FAILED_ROLLED_BACK'}
    Save-State @{result='UPDATED';installedSha=$remote;previousSha=$local}
