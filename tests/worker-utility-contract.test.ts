...
describe('Worker Utility V1 contract',()=>{
 ...
 it('binds badges by worker title before position fallback',()=>{expect(binder).toContain('TitleMatches(workerId');expect(binder).toContain('!used.Contains(x.Hwnd)');expect(binder).toContain('[{workerId} |');});
