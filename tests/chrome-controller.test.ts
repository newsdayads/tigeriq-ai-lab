...
function baseConfig():ControllerConfig{return{
  ...
  autopilot:{enabled:true,pollIntervalMs:15000,stateUrl:'http://127.0.0.1:8794/api/ui-autopilot/snapshot',requestTimeoutMs:5000,maxSnapshotAgeMs:300000,dispatchLeaseTtlMs:300000},
  recovery:{heartbeatStaleMs:90000,checkIntervalMs:15000,maxReopenAttempts:2,reopenBackoffMs:15000,startupReadyUrl:'http://127.0.0.1:8795/health',startupReadyTimeoutMs:120000,startupAttachGraceMs:20000,launchBrokerUrl:'http://127.0.0.1:8800',launchBrokerTimeoutMs:5000},
  workers:[
...
describe('NV02 completion watcher/autopilot',()=>{
  ...
  it('fails closed when a UI assignment is not Core-selected',()=>{const legacy=snapshot({source:'GITHUB',revision:'github-ui-v2:legacy',nextJob:{jobId:'GH-969',workerId:'NV02',status:'READY',executable:true,priority:'P0',prompt:'x'}});expect(decideAutoContinue(legacy,{...freshAutopilotState(),lastDispatchedJobId:'JOB-1',lastDispatchedAt:'2026-09-15T00:59:58.000Z'},NOW)).toMatchObject({kind:'STOP',reason:'NEXT_JOB_NOT_TRUSTED_SELECTION'});});
...
describe('Worker-generic UI controller contract and safety requirements',async()=>{it('supports independent profiles, random prompt selection from canonical pool, independent reloads, staggered resets, per-worker logging, and pause precedence',async()=>{const{CONTINUE_PROMPTS,pickWorkerPrompt,getWorkerProfilePath}=await import('../apps/chrome-controller/extension/continuity.js');expect(CONTINUE_PROMPTS.length).toBe(21);const p1=pickWorkerPrompt();expect(CONTINUE_PROMPTS).toContain(p1);const p2=pickWorkerPrompt();expect(CONTINUE_PROMPTS).toContain(p2);const workerIds=['NV02','NV03','NV04'];expect(workerIds).toHaveLength(3);expect(getWorkerProfilePath('NV02')).toContain('NV02');expect(getWorkerProfilePath('NV03')).toContain('NV03');expect(getWorkerProfilePath('NV04')).toContain('NV04');});});
