...
function baseConfig():ControllerConfig{return{
  ...
  autopilot:{enabled:true,pollIntervalMs:15000,requestTimeoutMs:5000,maxSnapshotAgeMs:300000},
...
function snapshot(overrides:Partial<ExternalAutopilotSnapshot>={}):ExternalAutopilotSnapshot{return{
  ...
  previousJob:{jobId:'JOB-1',workerId:'NV05',status:'DONE',executable:true,priority:'P0',evidence:[{source:'GITHUB',ref:'https://github.com/newsdayads/tigeriq-ai-lab/issues/763#evidence',verifiedAt:OBSERVED_AT}]},
...
describe('layout 5-3-4',()=>{
  ...
  it('falls back safely when a heartbeat reports an undersized display',()=>{const p=computePlacements(baseConfig(),{left:0,top:0,width:1024,height:768});expect(p.NV05.left).toBe(2572);expect(p.NV03.left).toBe(3080);expect(p.NV04.left).toBe(3588);});
});

describe('config and session guardrails',()=>{
  it('accepts canonical order and defaults enabled',()=>{const c=validateConfig(baseConfig());expect(c.workers.map(w=>w.id)).toEqual(['NV05','NV03','NV04']);expect(c.workers.map(w=>w.enabled)).toEqual([true,true,true]);expect(c.workers.every(isWorkerEnabled)).toBe(true);});
...
describe('NV05 completion watcher/autopilot',()=>{
  it('uses fixed AUTO_CONTINUE only after DONE + fresh external evidence',()=>{expect(AUTO_CONTINUE).toBe('AUTO_CONTINUE');expect(decideAutoContinue(snapshot(),freshAutopilotState(),NOW)).toMatchObject({kind:'DISPATCH',trigger:'AUTO_CONTINUE',jobId:'JOB-2'});});
  it('waits for running or missing evidence',()=>{expect(decideAutoContinue(snapshot({previousJob:{jobId:'J1',workerId:'NV05',status:'RUNNING',executable:true,priority:'P0'}}),freshAutopilotState(),NOW)).toMatchObject({kind:'BUSY'});expect(decideAutoContinue(snapshot({previousJob:{jobId:'J1',workerId:'NV05',status:'DONE',executable:true,priority:'P0'}}),freshAutopilotState(),NOW)).toMatchObject({kind:'WAIT_EVIDENCE'});});
  it('stops after FAILED/BLOCKED/CANCELLED instead of continuing',()=>{for(const status of ['FAILED','BLOCKED','CANCELLED'] as const){expect(decideAutoContinue(snapshot({previousJob:{jobId:'J1',workerId:'NV05',status,executable:true,priority:'P0',evidence:[{source:'GITHUB',ref:'x',verifiedAt:OBSERVED_AT}]}}),freshAutopilotState(),NOW)).toMatchObject({kind:'STOP',reason:`PREVIOUS_JOB_${status}`});}});
...
describe('review evidence and extension lifecycle',()=>{
  it('exports visible-only session policy and no stealth/output parsing',()=>{const evidence=buildRuntimeEvidence({config:baseConfig(),workArea:{left:0,top:0,width:4096,height:2120},workers:baseConfig().workers.map(w=>({id:w.id,enabled:true,status:'READY',blocked:false})),autopilot:freshAutopilotState(),snapshot:snapshot(),paused:false,killed:false,recoveryAttempts:{NV05:0,NV03:0,NV04:0},startupReady:true,interactiveSession:true,sessionName:'Console'});expect(evidence.layout.order).toEqual(['NV05','NV03','NV04']);expect(evidence.queue.globalUiConcurrency).toBe(1);expect(evidence.autopilot).toMatchObject({fixedTrigger:'AUTO_CONTINUE',aiOutputParsed:false});expect(evidence.sessionPolicy).toMatchObject({chromeVisibleOnly:true,interactiveSession:true,hiddenChromeAllowed:false});expect(evidence.security).toMatchObject({stealth:false,fakeHuman:false,credentialExtraction:false});});
  ...
  it('keeps badge repair idempotent and heartbeat alarm durable',()=>{const content=readFileSync('apps/chrome-controller/extension/content.js','utf8');const background=readFileSync('apps/chrome-controller/extension/background.js','utf8');expect(content).toContain('badge.textContent !== wantedText');expect(content).not.toContain('characterData: true');expect(background).toContain('ensureTickAlarm');expect(background).toContain("periodInMinutes:0.5");});
  it('forbids hidden Chrome and avoids retrying DISPATCH in source',()=>{const server=readFileSync('apps/chrome-controller/src/server.ts','utf8');expect(server).toContain('INTERACTIVE_SESSION_REQUIRED:NO_HIDDEN_CHROME');expect(server).toContain('windowsHide:false');expect(server).toContain("await sendCommand(workerId,'DISPATCH'");expect(server).not.toContain("runWithRetry(`${source.toLowerCase()}:${workerId}`");expect(server).toContain('AUTO_CONTINUE_COMMITTED');expect(server).toContain('pendingJobId');});
});
