...
describe('Worker-generic UI controller contract and safety requirements',()=>{it('supports independent profiles, random prompt selection from canonical pool, independent reloads, staggered resets, per-worker logging, and pause precedence',async()=>{const{CONTINUE_PROMPTS,pickWorkerPrompt}=await import('../apps/chrome-controller/extension/continuity.js');expect(CONTINUE_PROMPTS.length).toBe(21);const p1=pickWorkerPrompt();expect(CONTINUE_PROMPTS).toContain(p1);const p2=pickWorkerPrompt();expect(CONTINUE_PROMPTS).toContain(p2);});});
