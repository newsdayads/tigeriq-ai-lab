...
function baseConfig():ControllerConfig{return{
  ...
  workers:[
    {id:'NV02',role:'PRIMARY_AUTOPILOT_UI',profileDirectory:'Profile 5',homeUrl:'https://chatgpt.com/g/test-nv05'},
...
describe('config and session guardrails',()=>{
  it('accepts canonical order and defaults enabled',()=>{const c=validateConfig(baseConfig());expect(c.workers.map(w=>w.id)).toEqual(['NV02','NV03','NV04']);expect(c.workers.map(w=>w.enabled)).toEqual([true,true,true]);expect(c.workers.every(isWorkerEnabled)).toBe(true);});
...
describe('review evidence and extension lifecycle',()=>{
  it('exports visible-only session policy and no stealth/output parsing',()=>{const evidence=buildRuntimeEvidence({config:baseConfig(),workArea:{left:0,top:0,width:4096,height:2120},workers:baseConfig().workers.map(w=>({id:w.id,enabled:true,status:'READY',blocked:false})),autopilot:freshAutopilotState(),snapshot:snapshot(),paused:false,killed:false,recoveryAttempts:{NV02:0,NV03:0,NV04:0},startupReady:true,interactiveSession:true,sessionName:'Console'});expect(evidence.layout.order).toEqual(['NV02','NV03','NV04']);expect(evidence.queue.globalUiConcurrency).toBe(1);expect(evidence.autopilot).toMatchObject({fixedTrigger:'AUTO_CONTINUE',aiOutputParsed:false});expect(evidence.sessionPolicy).toMatchObject({chromeVisibleOnly:true,interactiveSession:true,hiddenChromeAllowed:false});expect(evidence.security).toMatchObject({stealth:false,fakeHuman:false,credentialExtraction:false});});
