...
describe('App Chrome runtime isolation',()=>{
  it('runtime disables external work autopilot regardless of legacy config',()=>{const server=readFileSync('apps/chrome-controller/src/server.ts','utf8');expect(server).toContain('const externalWorkAutopilotEnabled=false');expect(server).toContain('externalWorkAutopilotEnabled,');expect(server).toContain("sendCommand(workerId,'LOCAL_CONTINUE_NOW')");});
});
