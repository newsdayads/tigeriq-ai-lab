...
describe('NV02 continuity policy', () => {
  ...
  it('classifies ChatGPT request-timeout retry cards as recoverable chat errors', () => {
    ...
    expect(ui).toContain('const retryButtons=');
    expect(ui).toContain('retryContextFor');
    expect(ui).toContain('const timeoutRetry=');
    expect(ui).toContain('const chatRetry=timeoutRetry||conversationRetry');
    ...
    const retryExpr=source.slice(source.indexOf('function chatLoadRetryExpr'),source.indexOf('function loadContinuityFor'));
    expect(retryExpr).toContain("preferred.at(-1)?.button");
    expect(retryExpr).toContain("secondary.at(-1)?.button");
    expect(retryExpr).toContain("CHAT_LOAD_RETRY_NOT_UNIQUE");
