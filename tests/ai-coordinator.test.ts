...
describe('WO-043 AI coordinator', () => {
  it('uses the lowest-cost capable executor then independent verification', async () => {
    ...
    expect(result.executor?.target).not.toEqual(result.reviewer?.target);
  ...
  it('fails closed when strict independence cannot be satisfied', async () => {
    const store = new InMemoryCheckpointStore();
    ...
    const result = await coordinator.run(work({ id: 'WO-INDEP', kind: 'coding', risk: 'high' }));
