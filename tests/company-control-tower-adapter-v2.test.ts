...
// @ts-ignore Browser-side ESM module is intentionally exercised directly by Vitest.
import {
...
// @ts-ignore Browser-side mock module is intentionally exercised directly by Vitest.
...
describe('Company Control Tower Business State V2 adapter', () => {
  it('binds the read model to PR153 exact contract and keeps preview non-authoritative', () => {
    ...
    expect(vm.goals[0]).toEqual(expect.objectContaining({
      goalId: 'GOAL-COMPANY-001',
      status: 'RUNNING',
      lifecycleSource: 'PR141_OPERATIONAL_GOAL',
      isMock: true,
    }));
    ...
    expect(vm.employees.find((row: any) => row.employeeId === 'EMP-CHIEF-001')).toEqual(expect.objectContaining({
      businessRole: 'Chief of Staff AI',
      autonomyLevel: 'A2',
      isMock: true,
    }));
    expect(vm.ownerActions[0]).toEqual(expect.objectContaining({
      exceptionId: 'EXC-MOCK-001',
      status: 'AWAITING_OWNER',
      requiredOwnerAction: expect.stringContaining('Quyết định'),
    }));
  ...
  it('derives KPI current value only from the newest observation and preserves provenance', () => {
      ...
      kpi_observations: [
        {
          ...
          provenance: { source_system: 'crm', source_ref: 'crm://old', observed_at: '2026-09-02T10:00:00+07:00' },
        ...
        ...MOCK_CONTROL_TOWER_PREVIEW.kpi_observations,
        {
          ...
          kpi_id: 'KPI-TOP3',
          value: 3,
          observed_at: '2026-09-02T17:00:00+07:00',
          provenance: { source_system: 'crm', source_ref: 'crm://new', observed_at: '2026-09-02T17:00:00+07:00' },
    ...
    expect(kpi).toEqual(expect.objectContaining({
      currentValue: 3,
  ...
  it('accepts contract-shaped live projection, ignores preview, and does not accept legacy Web aliases', () => {
    const live = buildCompanyControlTowerViewModel(liveSnapshot({ businessStateV2: MOCK_CONTROL_TOWER_PREVIEW }), {
      previewBusiness: {
        goal_profiles: [{ goal_id: 'SHOULD-NOT-LEAK', title: 'preview', owner_ref: 'x', related_kpi_ids: [], updated_at: '2026-09-02T00:00:00Z' }],
      },
    });
    ...
    const legacy = buildCompanyControlTowerViewModel(liveSnapshot({
      businessState: {
        goals: [{ goalId: 'LEGACY-GOAL' }],
        missions: [{ missionId: 'LEGACY-MISSION' }],
      },
    }));
  ...
  it('keeps Mission to Job reference-only and leaves Job/Lease/Result/Evidence in Technical Operations', () => {
    ...
    expect(mission.jobRefs).toEqual([
      expect.objectContaining({ jobId: 'MOCK-JOB-RESEARCH', relation: 'execution' }),
      expect.objectContaining({ jobId: 'MOCK-JOB-SCORING', relation: 'verification' }),
    ]);
  ...
  it('does not create shadow finance state when CRM/accounting projection is absent', () => {
    ...
    expect(vm.performance.metrics).toEqual(expect.arrayContaining([
      expect.objectContaining({ key: 'revenue', value: null, state: 'NO_SOURCE' }),
      expect.objectContaining({ key: 'cost', value: null, state: 'NO_SOURCE' }),
    ]));
