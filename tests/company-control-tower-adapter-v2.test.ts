...
describe('Company Control Tower Business State V2 adapter', () => {
  ...
  it('keeps external workboard removed even when a legacy Trello-shaped payload is present', () => {
    const absent = buildCompanyControlTowerViewModel(MOCK_CONTROLLER_SNAPSHOT, { previewBusiness: MOCK_CONTROL_TOWER_PREVIEW });
    expect(absent.workCoordination).toEqual(expect.objectContaining({ available:false, reason:'EXTERNAL_WORKBOARD_REMOVED_BY_OWNER_DECISION', sourceSystem:null, cards:[] }));
    ...
    const live = buildCompanyControlTowerViewModel(liveSnapshot({
      workCoordination: {
        ...
        cards:[{cardId:'trello-1',title:'Legacy card must not project',status:'in_progress'}],
      },
    }));
    expect(live.workCoordination).toEqual(expect.objectContaining({ available:false, reason:'EXTERNAL_WORKBOARD_REMOVED_BY_OWNER_DECISION', sourceSystem:null, cards:[] }));
  });
